...
<body>
    <div class="register-container card">
        <div class="mapita-chip">
            <strong>Mapita Registro</strong>
            Tu cuenta te habilita a publicar, gestionar y crecer dentro del ecosistema Mapita.
        </div>
        ...
        <form method="post" action="">
            ...
            <div class="form-group">
                ...
                <div class="email-tools">
                    <button type="button" id="email-insert-at">Insertar @</button>
                    <button type="button" id="email-copy-at">Copiar @</button>
                    <small id="email-at-status">Atajo para celulares.</small>
                </div>
    ...
    <script>
        (function () {
            ...
            const btnInsert = document.getElementById('email-insert-at');
            const btnCopy = document.getElementById('email-copy-at');
            const status = document.getElementById('email-at-status');
            ...
            function setStatus(text, ok) {
                ...
                setTimeout(() => {
                    status.textContent = 'Atajo para celulares.';
                    status.style.color = '#64748b';
                }, 1500);
            }

            if (btnInsert && emailInput) {
                btnInsert.addEventListener('click', function () {
                    ...
                    const before = emailInput.value.slice(0, start);
                    const after = emailInput.value.slice(end);
                    emailInput.value = before + '@' + after;
                    const pos = start + 1;
                    emailInput.focus();
                    emailInput.setSelectionRange(pos, pos);
                    setStatus('Insertado @', true);
                });
            }

            if (btnCopy) {
                btnCopy.addEventListener('click', function () {
                    const done = () => setStatus('Copiado @', true);
                    const fail = () => setStatus('No se pudo copiar', false);

                    if (navigator.clipboard && navigator.clipboard.writeText) {
                        navigator.clipboard.writeText('@').then(done).catch(fail);
                        return;
                    }
