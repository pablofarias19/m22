...
class Encuesta {
     ...
     * @param array $preguntas  Array de ['texto'=>..., 'tipo'=>..., 'requerida'=>..., 'opciones'=>[...]]
     * @return bool
     */
    public static function savePreguntas($encuesta_id, array $preguntas) {
        ...
        try {
            ...
            foreach ($preguntas as $p) {
                ...
                $tipo     = in_array($p['tipo'] ?? '', $tipos_validos) ? $p['tipo'] : 'opcion_multiple';
                $requerida = (int)(bool)($p['requerida'] ?? 0);

                // Tipos sin opciones: lista vacía
                if (in_array($tipo, $tipos_sin_opciones)) {
                    $opts_str = '';
                } else {
                    $opts = array_filter(array_map('trim', (array)($p['opciones'] ?? [])), 'strlen');
                    $opts = array_slice(array_values($opts), 0, 8);
                    $opts_str = implode(',', $opts);
                }

                // Intentar con columna requerida (migración 038)
                try {
                    $stmt = $db->prepare(
                        "INSERT INTO preguntas_encuesta
                            (encuesta_id, texto_pregunta, tipo, opciones, requerida, orden)
                         VALUES (?, ?, ?, ?, ?, ?)"
                    );
                    $stmt->execute([$encuesta_id, $texto, $tipo, $opts_str, $requerida, $orden]);
                } catch (\PDOException $e2) {
                    // Fallback: columna requerida aún no existe
                    ...
                    $stmt->execute([$encuesta_id, $texto, $tipo, $opts_str, $orden]);
                }
                $orden++;
            }
            return true;
