...
class Noticia {
    ...
    public static function create($data) {
        try {
            ...
            // Intentar con columnas link + youtube_link (agregadas via migraciones)
            try {
                $sql = "INSERT INTO noticias
                        (titulo, contenido, categoria, imagen, link, youtube_link, user_id, activa, fecha_publicacion, created_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
                $stmt = $db->prepare($sql);
                $result = $stmt->execute([
                    $data['titulo'] ?? '',
                ...
                // Fallback progresivo si las columnas aún no existen
                if ($e->getCode() !== '42S22' && (int)($e->errorInfo[1] ?? 0) !== 1054) throw $e;
                try {
                    $sql = "INSERT INTO noticias
                            (titulo, contenido, categoria, imagen, link, user_id, activa, fecha_publicacion, created_at)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())";
                    $stmt = $db->prepare($sql);
                    $result = $stmt->execute([
                        $data['titulo'] ?? '',
