...
class Evento {
    ...
    public static function create($data) {
        try {
            ...
            $hasLink     = self::columnExists($db, 'link');
            $hasIcono    = self::columnExists($db, 'icono');

            $cols   = ['titulo','descripcion','lat','lng','fecha','hora','organizador','youtube_link','ubicacion','categoria'];
            $vals   = [
                $data['titulo'] ?? null,
            ...
            if ($hasLink)     { $cols[] = 'link';     $vals[] = $data['link']     ?? null; }
            if ($hasIcono)    { $cols[] = 'icono';    $vals[] = $data['icono']    ?? null; }
            if ($hasMapitaId) { $cols[] = 'mapita_id';$vals[] = $data['mapita_id']?? null; }

            $cols[] = 'activo'; $vals[] = 1;

            $colStr = implode(', ', $cols);
            $phStr  = implode(', ', array_fill(0, count($vals), '?'));
            $sql    = "INSERT INTO eventos ($colStr, created_at) VALUES ($phStr, NOW())";
    ...
    public static function update($id, $data) {
        try {
            ...
            if (self::columnExists($db, 'link') && array_key_exists('link', $data)) {
                $update[] = "link = ?";
                $params[] = $data['link'];
            }
            if (self::columnExists($db, 'icono') && array_key_exists('icono', $data)) {
                $update[] = "icono = ?";
                $params[] = $data['icono'];
            }
    ...
    public static function deactivate($id) {
        try {
            $db = Database::getInstance()->getConnection();
            $stmt = $db->prepare("UPDATE eventos SET activo = 0, updated_at = NOW() WHERE id = ?");
    ...
    public static function activate($id) {
        try {
            $db = Database::getInstance()->getConnection();
            $stmt = $db->prepare("UPDATE eventos SET activo = 1, updated_at = NOW() WHERE id = ?");
            return $stmt->execute([$id]);
        } catch (Exception $e) {
            error_log("Error en Evento::activate - " . $e->getMessage());
            return false;
        }
    }
    ...
    public static function delete($id) {
        try {
            ...
            error_log("Error en Evento::delete - " . $e->getMessage());
            return false;
        }
    }
