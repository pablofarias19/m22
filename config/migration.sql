...
-- Noticias: link al artículo original (botón "Leer Completo" en el modal del mapa)
ALTER TABLE noticias ADD COLUMN IF NOT EXISTS link      VARCHAR(500)   NULL    COMMENT 'URL del artículo original (abre en nueva pestaña)';

-- ============================================================
-- 15. Encuestas — Geolocalización y campos que usa el código
--     Permite que las encuestas aparezcan como pins en el mapa
-- ============================================================

-- Coordenadas para el pin en el mapa
ALTER TABLE encuestas ADD COLUMN IF NOT EXISTS lat              DECIMAL(10,6)  NULL    COMMENT 'Latitud del pin en el mapa';
ALTER TABLE encuestas ADD COLUMN IF NOT EXISTS lng              DECIMAL(10,6)  NULL    COMMENT 'Longitud del pin en el mapa';
ALTER TABLE encuestas ADD INDEX  IF NOT EXISTS idx_enc_coords   (lat, lng);

-- Campo activo (el schema original usa `activa`, el código usa `activo`)
ALTER TABLE encuestas ADD COLUMN IF NOT EXISTS activo           TINYINT(1) NOT NULL DEFAULT 1  COMMENT '1 = encuesta activa y visible';

-- Fechas que usa la API y el modelo
ALTER TABLE encuestas ADD COLUMN IF NOT EXISTS fecha_creacion   DATETIME  NULL DEFAULT NULL    COMMENT 'Fecha de publicación';
ALTER TABLE encuestas ADD COLUMN IF NOT EXISTS fecha_expiracion DATE      NULL DEFAULT NULL    COMMENT 'Fecha de vencimiento (NULL = sin vencimiento)';

-- Link externo para render_mode = link
ALTER TABLE encuestas ADD COLUMN IF NOT EXISTS link             VARCHAR(500) NULL               COMMENT 'URL externa de la encuesta';
