...
<body>
    <div style="max-width: 600px; margin: 0 auto; padding: 30px 0;">
        ...
        <form method="post" action="<?= $editing ? 'brand_form.php?id=' . $brand['id'] : 'brand_form.php' ?>">
            ...
            <label>Ubicación:
                <div style="display:flex;gap:8px;align-items:center;">
                    <input type="text" name="ubicacion" id="ubicacion-input"
                           value="<?= $editing ? htmlspecialchars($brand['ubicacion']) : '' ?>"
                           placeholder="Dirección completa (ej: Av. Corrientes 1234, Buenos Aires)"
                           style="flex:1;">
                    <button type="button" id="btn-geocode"
                            onclick="geocodificarUbicacion()"
                            title="Buscar dirección en el mapa"
                            style="white-space:nowrap;padding:8px 14px;background:#1B3B6F;color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:13px;font-weight:600;flex-shrink:0;">
                        📍 Geolocalizar
                    </button>
                </div>
                <div id="geo-status" style="font-size:12px;margin-top:4px;min-height:18px;"></div>
            </label>
            <label>Ubicación en el mapa:
                ...
                <small style="color:#666;">Hacé clic en el mapa para ajustar la ubicación, o usá el botón "📍 Geolocalizar" para buscar por dirección.</small>
            </label>
            <label>Estado:
    ...
    <script>
        ...
        /**
         * Geocodifica el contenido del campo #ubicacion-input usando Nominatim (OpenStreetMap).
         * Coloca el pin en el mapa y rellena los campos lat/lng.
         */
        async function geocodificarUbicacion() {
            ...
            if (!address) {
                status.innerHTML = '<span style="color:#e53e3e;">⚠️ Escribí una dirección primero.</span>';
                return;
            }

            btn.disabled = true;
            btn.textContent = '⏳ Buscando…';
            status.innerHTML = '<span style="color:#6b7280;">Consultando OpenStreetMap…</span>';
            ...
            try {
                ...
                if (!data || data.length === 0) {
                    status.innerHTML = '<span style="color:#e53e3e;">❌ No se encontró esa dirección. Intentá con más detalle (ciudad, país).</span>';
                    return;
                }

                // Si hay varios resultados, mostrar selector; si hay uno, aplicar directo
                if (data.length === 1) {
                    aplicarCoordenadas(data[0]);
                } else {
                    mostrarSelectorResultados(data);
                }
            } catch (err) {
                status.innerHTML = '<span style="color:#e53e3e;">❌ Error de red al consultar. Verificá tu conexión.</span>';
            } finally {
                btn.disabled = false;
                btn.textContent = '📍 Geolocalizar';
            }
        }

        /** Aplica un resultado Nominatim: rellena lat/lng y mueve el mapa */
        function aplicarCoordenadas(result) {
            ...
            const sel = document.getElementById('geo-selector');
            if (sel) sel.remove();

            status.innerHTML = '<span style="color:#059669;">✓ Ubicación asignada: ' +
                               result.display_name.split(',').slice(0, 3).join(',') + '</span>';
        }

        /** Muestra un menú con los resultados cuando Nominatim devuelve varios */
        function mostrarSelectorResultados(results) {
            const status = document.getElementById('geo-status');
            // Limpiar selector anterior
            const old = document.getElementById('geo-selector');
            if (old) old.remove();

            const sel = document.createElement('div');
            sel.id = 'geo-selector';
            sel.style.cssText = 'background:#fff;border:1px solid #d1d5db;border-radius:8px;box-shadow:0 4px 12px rgba(0,0,0,.12);margin-top:6px;overflow:hidden;max-height:200px;overflow-y:auto;';

            results.slice(0, 5).forEach(function(r, i) {
                const item = document.createElement('div');
                item.style.cssText = 'padding:10px 14px;cursor:pointer;font-size:13px;border-bottom:1px solid #f3f4f6;';
                item.textContent = r.display_name;
                item.addEventListener('mouseenter', function() { this.style.background = '#eff6ff'; });
                item.addEventListener('mouseleave', function() { this.style.background = ''; });
                item.addEventListener('click', function() { aplicarCoordenadas(r); });
                sel.appendChild(item);
            });

            status.parentNode.insertBefore(sel, status.nextSibling);
            status.innerHTML = '<span style="color:#1d4ed8;">🔽 ' + results.length + ' resultados — seleccioná el correcto:</span>';
        }
