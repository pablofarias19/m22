...
<head>
    ...
    <style>
        ...
        /* ── FAB buscar localidad (encima del botón Leyenda) ────────── */
        #mapita-locality-fab {
            position: absolute;
            bottom: 122px;
            right: 10px;
        ...
        * { box-sizing: border-box; }
        ...
        .home-contact-card:hover { background: #667eea; }
        .quickstart-panel {
        ...
        .sb-info-btn:hover { background: rgba(102,126,234,0.12); }
        ...
        .loc-mode-btn--active {
            background: #667eea;
            color: white;
            box-shadow: 0 1px 4px rgba(102,126,234,0.35);
        }
...
<div id="sidebar">
    ...
    <div id="search-results-dropdown" style="display:none;max-height:200px;overflow-y:auto;background:#fff;border:1px solid #d0d5dd;border-radius:8px;margin-bottom:12px;box-shadow:0 2px 8px rgba(0,0,0,0.1);z-index:1000;">
    </div>
    ...
    <div class="sb-section" id="sb-sec-filters">
        <div class="sb-section-hdr open" onclick="toggleSbSection(this)"
            ...
            <div style="display:flex;align-items:center;gap:4px;margin-left:auto;">
                <button type="button" class="sb-info-btn" onclick="event.stopPropagation();toggleInfoPanel('filters')" aria-label="Ayuda sobre filtros">ℹ️</button>
    ...
    <div id="info-filters" class="sb-info-panel" style="display:none;">
        Filtrá negocios y marcas por tipo, horario, protección, país e idioma. Cada filtro se aplica en combinación con los demás. Usá el buscador superior para búsquedas por nombre.
    </div>
    ...
    <div class="sb-section" id="sb-sec-location">
        <div class="sb-section-hdr" onclick="toggleSbSection(this)"
             aria-expanded="false" title="Ubicación y cercanía" data-tooltip="Ubicación">
            <span class="sb-section-hdr-label">
                <span aria-hidden="true">📍</span> Ubicación y Cercanía
            </span>
            <div style="display:flex;align-items:center;gap:4px;margin-left:auto;">
                <button type="button" class="sb-info-btn" onclick="event.stopPropagation();toggleInfoPanel('location')" aria-label="Ayuda sobre ubicación">ℹ️</button>
                <svg class="sb-chevron" width="15" height="15" viewBox="0 0 24 24" fill="none"
                     stroke="currentColor" stroke-width="2.5" stroke-linecap="round"
                     stroke-linejoin="round" aria-hidden="true">
                    <polyline points="6 9 12 15 18 9"/>
        ...
        <div class="sb-section-body">
            <div class="sb-section-body-inner">
                <div id="info-location" class="sb-info-panel" style="display:none;">
                    Establece tu posición en el mapa. Usa <strong>GPS</strong> real o coloca un <strong>punto virtual</strong> para simular tu ubicación y filtrar resultados por cercanía. El radio define el área de búsqueda.
                </div>
                <!-- Mode toggle GPS / Virtual -->
                <div class="loc-mode-row">
                    <button id="loc-btn-gps" type="button" class="loc-mode-btn loc-mode-btn--active" onclick="setLocationMode('gps')">📡 GPS</button>
                    <button id="loc-btn-virtual" type="button" class="loc-mode-btn" onclick="setLocationMode('virtual')">🗺️ Virtual</button>
                </div>
                <!-- GPS actions -->
                <div id="loc-gps-actions" style="display:flex;gap:5px;margin-top:6px;">
                    <button onclick="ubicarme()"
                            aria-label="Ubicar mi posición en el mapa"
                            style="flex:1;padding:9px 6px;background:#667eea;color:white;border:none;border-radius:6px;cursor:pointer;font-size:12px;">
                        📍 Ubicarme
                    </button>
                    <button id="btn-follow-me" onclick="toggleFollowMe()"
                            aria-pressed="false"
                            aria-label="Seguir mi posición en el mapa"
                            style="flex:1;padding:9px 6px;background:#95a5a6;color:white;border:none;border-radius:6px;cursor:pointer;font-size:12px;">
                        📡 Seguirme
                    </button>
                </div>
                <!-- Virtual actions (hidden by default) -->
                <div id="loc-virtual-actions" style="display:none;margin-top:6px;">
                    <button id="btn-place-virtual" onclick="activateVirtualPlacement()"
                            style="width:100%;padding:9px 6px;background:#0891b2;color:white;border:none;border-radius:6px;cursor:pointer;font-size:12px;margin-bottom:5px;">
                        📌 Colocar punto virtual
                    </button>
                    <button id="btn-recenter-virtual" onclick="recentrarVirtual()"
                            style="display:none;width:100%;padding:9px 6px;background:#0f766e;color:white;border:none;border-radius:6px;cursor:pointer;font-size:12px;">
                        🎯 Recentrar punto virtual
                    </button>
                </div>
                <!-- Radius selector (moved here from Resultados) -->
                <div style="display:flex;align-items:center;gap:6px;margin-top:8px;padding-top:8px;border-top:1px solid #eef0f5;">
                    <label id="sb-radius-lbl" style="font-size:11px;color:#555;white-space:nowrap;" for="sb-radius-select">📏 Radio:</label>
                    <select id="sb-radius-select" onchange="filtrar()" aria-labelledby="sb-radius-lbl" style="font-size:11px;padding:3px 6px;border:1px solid #d0d5dd;border-radius:4px;flex:1;">
                ...
                <!-- Quitar ubicación (visible solo cuando hay ubicación activa) -->
                <div id="loc-clear-row" style="display:none;margin-top:6px;">
                    <button onclick="clearLocation()"
                            style="width:100%;padding:8px 6px;background:transparent;color:#6b7280;border:1px solid #d1d5db;border-radius:6px;cursor:pointer;font-size:11px;font-weight:600;letter-spacing:0.2px;">
                        ✕ Quitar ubicación
                    </button>
                </div>
            </div>
    ...
    <div class="sb-section" id="sb-sec-tools">
        <div class="sb-section-hdr" onclick="toggleSbSection(this)"
            ...
            <div style="display:flex;align-items:center;gap:4px;margin-left:auto;">
                <button type="button" class="sb-info-btn" onclick="event.stopPropagation();toggleInfoPanel('tools')" aria-label="Ayuda sobre herramientas">ℹ️</button>
                <svg class="sb-chevron" width="15" height="15" viewBox="0 0 24 24" fill="none"
                     stroke="currentColor" stroke-width="2.5" stroke-linecap="round"
                     stroke-linejoin="round" aria-hidden="true">
                    <polyline points="6 9 12 15 18 9"/>
        ...
        <div class="sb-section-body">
            <div class="sb-section-body-inner">
                <div id="info-tools" class="sb-info-panel" style="display:none;">
                    Acciones adicionales sobre la lista de resultados. <strong>Exportar PDF</strong> genera un documento con los resultados actualmente visibles en el panel.
                </div>
                <button onclick="exportarPDF()"
                        aria-label="Exportar lista como PDF"
                        style="width:100%;padding:10px;background:#667eea;color:white;border:none;border-radius:6px;cursor:pointer;font-size:12px;">
                    🧾 Exportar PDF
                </button>
            </div>
        </div>
            ...
            <div class="sb-section-body-inner">
                ...
                <div class="cq-acc-body open" id="cq-grp-consultas">
                    <div class="cq-acc-body-inner">
                        ...
                        <button type="button" id="btn-consulta-envio" class="cq-btn cq-btn--envio"
                ...
                <div class="cq-acc-body open" id="cq-grp-inmobiliarias">
                    ...
                    <div style="margin-top:10px;border-top:1px solid #e5e7eb;padding-top:8px;">
                        <div id="inm-f-toggle" onclick="toggleInmFiltros()"
                             style="display:flex;align-items:center;justify-content:space-between;cursor:pointer;padding:6px 9px;background:#f9fafb;border-radius:9px;border:1px solid #e5e7eb;user-select:none;-webkit-tap-highlight-color:transparent;">
                            <span style="font-size:11px;font-weight:700;color:#374151;">🎛️ Filtros CERCA</span>
                            <span id="inm-f-arrow" style="font-size:10px;color:#9ca3af;transition:transform .2s;">▼</span>
                        </div>
                        <div id="inm-f-body" style="display:none;margin-top:7px;padding:10px;background:#f9fafb;border-radius:9px;border:1px solid #e5e7eb;">

                            <!-- Operación -->
                            <div style="margin-bottom:10px;">
                                <div style="font-size:10px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.4px;margin-bottom:5px;">Operación</div>
                                <div style="display:flex;gap:3px;flex-wrap:wrap;" id="inm-f-op-grp">
                                    <button class="inm-f-btn inm-f-btn--on" data-op="" onclick="setInmOp(this,'')">Todas</button>
                                    <button class="inm-f-btn" data-op="venta" onclick="setInmOp(this,'venta')">🏠 Venta</button>
                                    <button class="inm-f-btn" data-op="alquiler" onclick="setInmOp(this,'alquiler')">🔑 Alquiler</button>
                                    <button class="inm-f-btn" data-op="proyecto" onclick="setInmOp(this,'proyecto')">🏗️ Proyecto</button>
                                </div>
                            </div>

                            <!-- Precio -->
                            <div style="margin-bottom:10px;">
                                <div style="font-size:10px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.4px;margin-bottom:5px;">Precio (USD)</div>
                                <div style="display:flex;gap:5px;align-items:center;">
                                    <input type="number" id="inm-f-pmin" placeholder="Mínimo" min="0"
                                           style="flex:1;min-width:0;padding:6px 8px;border:1px solid #d1d5db;border-radius:7px;font-size:12px;font-family:inherit;outline:none;background:#fff;">
                                    <span style="color:#9ca3af;font-size:13px;flex-shrink:0;font-weight:600;">—</span>
                                    <input type="number" id="inm-f-pmax" placeholder="Máximo" min="0"
                                           style="flex:1;min-width:0;padding:6px 8px;border:1px solid #d1d5db;border-radius:7px;font-size:12px;font-family:inherit;outline:none;background:#fff;">
                                </div>
                                <div style="display:flex;gap:3px;margin-top:5px;flex-wrap:wrap;">
                                    <button onclick="setInmPreset(0,80000)"  class="inm-f-preset">≤80k</button>
                                    <button onclick="setInmPreset(0,150000)" class="inm-f-preset">≤150k</button>
                                    <button onclick="setInmPreset(0,300000)" class="inm-f-preset">≤300k</button>
                                    <button onclick="setInmPreset(0,0)"      class="inm-f-preset inm-f-preset--clear">✕ Limpiar</button>
                                </div>
                            </div>

                            <button onclick="aplicarInmFiltros()"
                                    style="width:100%;padding:8px;background:#1B3B6F;color:#fff;border:none;border-radius:9px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;transition:background .15s;"
                                    onmouseover="this.style.background='#14306b'" onmouseout="this.style.background='#1B3B6F'">
                                ✓ Aplicar filtros
                            </button>

                            <!-- Badge contador de resultados -->
                            <div id="inm-f-count" style="margin-top:6px;font-size:11px;color:#6b7280;text-align:center;display:none;"></div>
                        </div>
                    </div>
                </div>
    ...
    <div id="eventos-container" class="sidebar-card" style="display:none;border-left:4px solid #e74c3c;padding:0;overflow:hidden;">
        <div class="sb-mod-hdr open" onclick="toggleSbModule(this)">
            <h3 style="margin:0;font-size:13px;color:#e74c3c;font-weight:700;">🎉 Eventos</h3>
    ...
    <div class="sb-section" id="sb-sec-results">
        <div class="sb-section-hdr open" onclick="toggleSbSection(this)" aria-expanded="true"
            ...
            <span class="sb-section-hdr-label"></span>
            <div style="display:flex;align-items:center;gap:6px;flex-shrink:1;min-width:0;overflow:hidden;">
                ...
                <button type="button" class="sb-info-btn" onclick="event.stopPropagation();toggleInfoPanel('results')" aria-label="Ayuda sobre resultados">ℹ️</button>
                <svg class="sb-chevron" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="6 9 12 15 18 9"/></svg>
            </div>
        </div>
        <div class="sb-section-body open">
            <div id="info-results" class="sb-info-panel" style="display:none;margin:4px 0 2px;">
                Muestra los resultados filtrados ordenados por cercanía. Al escribir en el buscador, el mapa se centra en el primer resultado. Ajustá el <strong>máx. ítems</strong> y el <strong>zoom</strong> según tu preferencia.
            </div>
            <!-- Search preferences accordion -->
            <div class="accordion-item" style="border-bottom:1px solid #eef0f5;">
                <button class="accordion-btn" onclick="toggleAccordion(this)" style="font-size:11px;padding:6px 10px;background:#fafbff;">
                    ⚙️ Preferencias de búsqueda
                    <span style="font-size:10px;">▼</span>
                </button>
                <div class="accordion-content">
                    <div style="display:flex;flex-direction:column;gap:6px;padding:4px 0;">
                        <label style="font-size:11px;color:#374151;display:flex;align-items:center;justify-content:space-between;gap:8px;">
                            Máx. ítems dropdown
                            <select id="pref-max-items" onchange="saveSearchPrefs()" style="font-size:11px;padding:2px 6px;border:1px solid #d0d5dd;border-radius:4px;width:70px;">
                        ...
                        <label style="font-size:11px;color:#374151;display:flex;align-items:center;justify-content:space-between;gap:8px;">
                            Zoom primer resultado
                            <select id="pref-zoom-first" onchange="saveSearchPrefs()" style="font-size:11px;padding:2px 6px;border:1px solid #d0d5dd;border-radius:4px;width:70px;">
                        ...
                        <label style="font-size:11px;color:#374151;display:flex;align-items:center;justify-content:space-between;gap:8px;">
                            Zoom al seleccionar
                            <select id="pref-zoom-selected" onchange="saveSearchPrefs()" style="font-size:11px;padding:2px 6px;border:1px solid #d0d5dd;border-radius:4px;width:70px;">
                                <option value="12">12</option>
                                <option value="13">13</option>
                                <option value="14">14</option>
                                <option value="15">15</option>
    ...
    <!-- ── CAPAS DE TERRITORIO ────────────────────────────────────────────── -->
    <div class="sb-section" id="sb-sec-layers">
        <div class="sb-section-hdr" onclick="toggleSbSection(this)"
             aria-expanded="false" title="Capas de territorio" data-tooltip="Capas">
            <span class="sb-section-hdr-label">
                <span aria-hidden="true">🗂️</span> Capas de Territorio
            </span>
            <div style="display:flex;align-items:center;gap:4px;margin-left:auto;">
                <button type="button" class="sb-info-btn" onclick="event.stopPropagation();toggleInfoPanel('layers')" aria-label="Ayuda sobre capas">ℹ️</button>
                <svg class="sb-chevron" width="15" height="15" viewBox="0 0 24 24" fill="none"
                     stroke="currentColor" stroke-width="2.5" stroke-linecap="round"
                     stroke-linejoin="round" aria-hidden="true">
                    <polyline points="6 9 12 15 18 9"/>
        ...
        <div class="sb-section-body">
            <div class="sb-section-body-inner">
                <div id="info-layers" class="sb-info-panel" style="display:none;">
                    Activa capas adicionales sobre el mapa. <strong>Zonas inmobiliarias</strong> muestra áreas de influencia de inmobiliarias. <strong>Sectores Industriales</strong> superpone áreas de actividad industrial. Podés activar ambas al mismo tiempo.
                </div>
                <!-- Zonas Inmobiliarias -->
                <div id="inmuebles-container" style="padding:4px 0 6px;">
                    <label style="display:flex;align-items:center;cursor:pointer;font-size:12px;gap:8px;">
                        <input type="checkbox" id="show-inmuebles" onchange="toggleInmuebles()" style="width:16px;height:16px;flex-shrink:0;">
                        🏠 Zonas de influencia (inmobiliarias)
                    </label>
                </div>
                <!-- Sectores Industriales -->
                <div id="sectores-industriales-container" style="padding:4px 0 2px;border-top:1px solid #eef0f5;">
                    <label style="display:flex;align-items:center;cursor:pointer;font-size:12px;gap:8px;margin-top:6px;">
                        <input type="checkbox" id="show-sectores-industriales" onchange="toggleSectoresIndustriales()" style="width:16px;height:16px;flex-shrink:0;">
                        🏭 Sectores Industriales
                    </label>
                </div>
            </div>
    ...
    <!-- Auth -->
    <div id="sidebar-auth-section" class="sidebar-card" style="margin-top:8px;">
        <div style="margin-bottom:8px;padding:8px 10px;border-radius:8px;background:linear-gradient(135deg,#0f172a,#1e3a8a);color:#e5edff;border:1px solid rgba(147,197,253,.35);">
            <div style="display:flex;align-items:center;gap:6px;font-weight:700;font-size:12px;letter-spacing:.2px;">
                <span aria-hidden="true">🗺️</span>
                <span>Mapita Identity</span>
            </div>
            <div style="margin-top:3px;font-size:11px;opacity:.9;line-height:1.35;">Acceso rapido para sumarte al mapa y gestionar tu perfil.</div>
        </div>
        <?php if (!isset($_SESSION['user_id'])): ?>
            ...
            <div style="margin-top:6px;display:flex;align-items:center;gap:6px;">
                <button type="button" id="btn-copy-at-map"
                        onclick="copyAtSymbol()"
                        style="flex:0 0 auto;padding:7px 9px;background:#ffffff;border:1px solid #d1d5db;border-radius:6px;color:#374151;font-size:11px;font-weight:700;cursor:pointer;">
                    Copiar @
                </button>
                <span id="copy-at-map-status" style="font-size:10px;color:#6b7280;line-height:1.2;">Atajo util para completar email en celular.</span>
            </div>
        <?php else: ?>
...
<!-- ── FAB buscar localidad ─────────────────────────────────────── -->
<button id="mapita-locality-fab" title="Buscar localidad · Search locality" aria-label="Buscar localidad en el mapa">🌍</button>
...
const UI_STRINGS = {
    es: {
        ...
        filter_search: 'Buscar negocios...', filter_all_types: 'Tipos de Negocios',
    ...
    en: {
        ...
        filter_search: 'Search businesses...', filter_all_types: 'Business Types',
    ...
    pt: {
        ...
        filter_search: 'Buscar negócios...', filter_all_types: 'Tipos de Negócios',
    ...
    fr: {
        ...
        filter_search: 'Rechercher entreprises...', filter_all_types: "Types d'entreprises",
    ...
    de: {
        ...
        filter_search: 'Unternehmen suchen...', filter_all_types: 'Unternehmenstypen',
    ...
    no: {
        ...
        filter_search: 'Søk bedrifter...', filter_all_types: 'Bedriftstyper',
    ...
    zh: {
        ...
        filter_search: '搜索商家...', filter_all_types: '商家类型',
    ...
    ar: {
        ...
        filter_search: 'البحث عن الأعمال...', filter_all_types: 'أنواع الأعمال',
    ...
    it: {
        ...
        filter_search: 'Cerca attività...', filter_all_types: 'Tipi di attività',
    ...
    ru: {
        ...
        filter_search: 'Поиск бизнеса...', filter_all_types: 'Типы бизнеса',
    ...
    el: {
        ...
        filter_search: 'Αναζήτηση επιχειρήσεων...', filter_all_types: 'Τύποι επιχειρήσεων',
    ...
    tr: {
        ...
        filter_search: 'İşletme ara...', filter_all_types: 'İşletme türleri',
    ...
    ja: {
        ...
        filter_search: 'ビジネス検索...', filter_all_types: 'ビジネスタイプ',
    ...
    ko: {
        ...
        filter_search: '비즈니스 검색...', filter_all_types: '비즈니스 유형',
    ...
    hi: {
        ...
        filter_search: 'व्यवसाय खोजें...', filter_all_types: 'व्यवसाय के प्रकार',
...
// ─── Location mode ────────────────────────────────────────────────────────────
let locationMode      = 'gps';   // 'gps' | 'virtual'
let virtualMarkerPos  = null;    // { lat, lng }
let virtualMarker     = null;    // Leaflet draggable marker
let _virtualPlacementActive = false;

// ─── Search preferences (persisted in localStorage) ───────────────────────────
let searchMaxItems    = 10;
let searchZoomFirst   = 15;
let searchZoomSelected = 16;

/** Returns the active user position regardless of GPS or virtual mode. */
function getEffectiveUserLocation() {
    if (locationMode === 'virtual' && virtualMarkerPos) return virtualMarkerPos;
    return miUbicacion;
}
...
const BUSINESS_FALLBACK_EMOJI = {
    ...
    optica:'👓', productora_audiovisual:'🎥', escuela_musicos:'🎼',
    taller_artes:'🎨', biodecodificacion:'🧬', libreria_cristiana:'📚',
    // Salud
    farmacia:'💊', hospital:'🏥', clinica:'🏥', veterinaria:'🐾',
    medico_pediatra:'🧒', medico_traumatologo:'🦴', laboratorio:'🧪',
    odontologia:'🦷', psicologo:'🧠', psicopedagogo:'📚',
    fonoaudiologo:'🗣️', grafologo:'✍️', enfermeria:'🩺', asistencia_ancianos:'🧓',
    // Belleza & Bienestar
    salon_belleza:'💇', peluqueria:'✂️', barberia:'💈',
    spa:'💆', gimnasio:'💪', gym:'🏋️', danza:'💃',
    // Servicios
    banco:'🏦', inmobiliaria:'🏠', seguros:'🛡️', abogado:'⚖️', agente_inpi:'📋',
    contador:'📊', arquitectura:'📐', ingenieria:'⚙️', ingenieria_civil:'🏗️',
    electricista:'💡', gasista:'🔥', gas_en_garrafa:'🛢️',
    seguridad:'🛡️', grafica:'🖨️', astrologo:'🔮',
    zapatero:'👞', videojuegos:'🎮', maestro_particular:'📘',
    alquiler_mobiliario_fiestas:'🪑', propalacion_musica:'🔊', animacion_fiestas:'🎉',
    taller:'🔩', taller_mecanico:'🔩', herreria:'🔨', carpinteria:'🪵',
    modista:'🧵', construccion:'🏗️', centro_vecinal:'🏘️', remate:'🔨',
    lavadero:'🧺', cerrajeria:'🔑',
    // Transporte
    transporte:'🚛', transporte_envios:'📦', transporte_pasajeros:'🚌',
    transporte_carga:'🚛', transportista:'🚚', logistica:'🏭', flota:'🚐',
    // Educación & Turismo
    academia:'🎓', educacion:'📚', idiomas:'🌐', escuela:'🏫',
    hotel:'🏨', turismo:'✈️', cine:'🎬', teatro:'🎭',
    // Arte & Cultura
    obra_de_arte:'🎭', musico:'🎸', cantante:'🎤', bailarin:'💃',
    actor:'🎭', actriz:'🎭', director_artistico:'🎬', guionista:'📝',
    escenografo:'🖼️', fotografo_artistico:'📷', productor_artistico:'🎙️',
    maquillador:'💄', pintor:'🎨', poeta:'📜', musicalizador:'🎵',
    editor_grafico:'🖥️', asistente_artistico:'🤝',
};
...
function inicializarMapa() {
    if (mapa) { mapa.remove(); }
    mapa = L.map('map').setView([-38.4161, -63.5989], 5);
...
function _filterCacheKey() {
    ...
    const radius = document.getElementById('sb-radius-select')?.value || '5';
    const locKey = locationMode + (getEffectiveUserLocation() ? ':1' : ':0');
    ...
    return `${texto}|${tipo}|${radius}|${locKey}|${open}|${ctry}|${lang}`;
}
...
function filtrar() {
    ...
    if (currentVer === 'negocios' || currentVer === 'ambos') {
        negocios_filtered = negocios.filter(n => {
            ...
            // Zoom filter desactivado: el mapa inicia en vista Argentina (zoom 5)
            // y el cluster de Leaflet maneja la densidad visual a cualquier zoom.
            ...
            if (locationFilter && _effLoc && n.lat && n.lng) {
                const dist = calcularDistancia(_effLoc.lat, _effLoc.lng, n.lat, n.lng);
    ...
    const effectiveLoc = getEffectiveUserLocation();
    ...
    if (effectiveLoc) {
        sidebarItems = items.filter(i => {
            ...
            return calcularDistancia(effectiveLoc.lat, effectiveLoc.lng, i.lat, i.lng) <= sbRadius;
        });
        sidebarItems.sort((a, b) =>
            calcularDistancia(effectiveLoc.lat, effectiveLoc.lng, a.lat, a.lng) -
            calcularDistancia(effectiveLoc.lat, effectiveLoc.lng, b.lat, b.lng)
        );
    ...
    const locModeLabel = locationMode === 'virtual' ? '🗺️' : '📍';
    const statsHTML =
        `<span class="stats-total">Total:&nbsp;${cntN + cntM}</span>`
        + `<span class="stats-sep">|</span>`
        + (effectiveLoc
            ? `<span class="stats-badge">${locModeLabel}&nbsp;${sidebarItems.length}&nbsp;cerca&nbsp;(${sbRadius}&nbsp;km)</span>`
            : isActiveSearch
...
function addListItem(container, n, searchTerm) {
    ...
    let dText = '';
    const _loc4dist = getEffectiveUserLocation();
    if (_loc4dist && n.lat && n.lng) {
        dText = ' · ' + calcularDistancia(_loc4dist.lat, _loc4dist.lng, n.lat, n.lng).toFixed(1) + ' km';
    }
...
function mostrarMarcadores(lista) {
    ...
    lista.forEach(n => {
        ...
        if (isMarca) {
            ...
            const iconData = iconosDB[n.business_type] || { emoji: BUSINESS_FALLBACK_EMOJI[n.business_type] || '📍', color: '#667eea' };
    ...
    updateSearchResultsDropdown(lista);
}

// ─── Actualizar dropdown de resultados de búsqueda ───────────────────────────────
function updateSearchResultsDropdown(lista) {
    const dropdown = document.getElementById('search-results-dropdown');
    if (!dropdown) return;
    
    const searchInput = document.getElementById('busqueda')?.value.trim() || '';
    
    // Solo mostrar dropdown si hay búsqueda activa
    if (!searchInput || !lista || lista.length === 0) {
        dropdown.innerHTML = '';
    ...
    if (lista.length > 0) {
        ...
        if (first.lat && first.lng) {
            mapa.setView([parseFloat(first.lat), parseFloat(first.lng)], searchZoomFirst);
        }
    }

    // Generar items del dropdown (configurable)
    const maxItems = Math.min(lista.length, searchMaxItems);
    let html = '';
    
    for (let i = 0; i < maxItems; i++) {
        const item = lista[i];
        const name = item.nombre || item.name || 'Sin nombre';
        const type = item.tipo === 'marca' ? '🏷️' : (item.business_type ? '🏢' : '📍');
        const display = type + ' ' + escapeHtml(name);
        
        html += `<div style="padding:10px 12px;border-bottom:1px solid #f0f0f0;cursor:pointer;transition:0.2s;font-size:13px;"
                     onmouseover="this.style.backgroundColor='#f5f5f5';"
                     onmouseout="this.style.backgroundColor='#fff';"
                     onclick="centerMapOnSearchResult(${parseFloat(item.lat)}, ${parseFloat(item.lng)}, '${escapeHtml(name).replace(/'/g, "\\'")}')">
                    ${display}
                    ${item.address ? '<div style="font-size:11px;color:#666;margin-top:3px;">📍 ' + escapeHtml(item.address) + '</div>' : ''}
                </div>`;
    }
    
    if (lista.length > maxItems) {
        html += `<div style="padding:8px;text-align:center;font-size:11px;color:#999;background:#f9f9f9;">
                    ... y ${lista.length - maxItems} resultado(s) más
                 </div>`;
    }
...
// ─── Centrar mapa en resultado de búsqueda seleccionado ──────────────────────────
function centerMapOnSearchResult(lat, lng, name) {
    mapa.setView([lat, lng], searchZoomSelected);
}
...
function buildPopup(n, isMarca) {
    ...
    if (isMarca) {
        ...
        // ── Address ──
        if (address) {
            p += '<div class="biz-address-block">';
            p += '<div class="biz-address-label">📍 Dirección</div>';
            p += '<div class="biz-address-val">' + escapeHtml(address) + '</div>';
            p += '</div>';
        }

        // ── Description (italic quote style, hidden on mobile) ──
        if (n.description && n.description.trim()) {
            const exc = n.description.length > 120 ? n.description.substring(0, 120) + '…' : n.description;
            p += '<p class="popup-desc biz-desc-quote">"' + escapeHtml(exc) + '"</p>';
        }

        // ── Details info card ──
        let infoCard = '';
        if (n.phone) {
            infoCard += '<div class="biz-info-row"><div class="biz-info-circle">📞</div><span>' + escapeHtml(n.phone) + '</span></div>';
        }
        if (n.business_type === 'comercio' && n.tipo_comercio) {
            infoCard += '<div class="biz-info-row"><div class="biz-info-circle">🏬</div><span>' + escapeHtml(n.tipo_comercio) + '</span></div>';
        }
        if (isVehicleSaleType(n)) {
            const vTipo = n.business_type === 'autos_venta' ? '🚗 Auto en venta' : '🏍️ Moto en venta';
            infoCard += '<div class="biz-info-row"><div class="biz-info-circle">🚘</div><span>' + vTipo + '</span></div>';
            const vModel = [n.vehiculo_marca, n.vehiculo_modelo].filter(Boolean).join(' ') || n.tipo_comercio || '';
            if (vModel) infoCard += '<div class="biz-info-row"><div class="biz-info-circle">🔧</div><span>' + escapeHtml(vModel) + '</span></div>';
            if (n.vehiculo_anio) infoCard += '<div class="biz-info-row"><div class="biz-info-circle">📅</div><span>Año ' + escapeHtml(String(n.vehiculo_anio)) + '</span></div>';
            if (n.vehiculo_km)   infoCard += '<div class="biz-info-row"><div class="biz-info-circle">🛣️</div><span>' + Number(n.vehiculo_km).toLocaleString() + ' km</span></div>';
            if (n.vehiculo_precio) infoCard += '<div class="biz-info-row"><div class="biz-info-circle" style="background:#fef3c7;">💰</div><span style="color:#d97706;font-weight:700;font-size:13px;">$' + Number(n.vehiculo_precio).toLocaleString() + '</span></div>';
            if (n.vehiculo_contacto) infoCard += '<div class="biz-info-row"><div class="biz-info-circle">📲</div><span>' + escapeHtml(n.vehiculo_contacto) + '</span></div>';
        } else if (isRemateType(n)) {
            const rs = getRemateStatus(n);
            infoCard += '<div class="biz-info-row"><div class="biz-info-circle">🔨</div><span>' + escapeHtml(rs.label) + '</span></div>';
            const rw = getRemateWindow(n);
            if (rw.inicio) infoCard += '<div class="biz-info-row"><div class="biz-info-circle">🕐</div><span>Inicio: ' + escapeHtml(formatDateTime(rw.inicio)) + '</span></div>';
            if (rw.fin)    infoCard += '<div class="biz-info-row"><div class="biz-info-circle">⏳</div><span>Cierre: ' + escapeHtml(formatDateTime(rw.fin)) + '</span></div>';
            if (n.remate_titulo || n.tipo_comercio) infoCard += '<div class="biz-info-row"><div class="biz-info-circle">📋</div><span>' + escapeHtml(n.remate_titulo || n.tipo_comercio) + '</span></div>';
        } else if (n.horario_apertura || n.horario_cierre) {
            const openHour  = truncateHourLabel(n.horario_apertura);
            const closeHour = truncateHourLabel(n.horario_cierre);
            const sched = (openHour && closeHour) ? (openHour + ' – ' + closeHour + ' hs') : (openHour || closeHour);
            if (sched) infoCard += '<div class="biz-info-row"><div class="biz-info-circle">🕐</div><span>' + escapeHtml(sched) + '</span></div>';
        }
        if (n.categorias_productos) {
            const tags = String(n.categorias_productos).split(',').map(t => t.trim()).filter(Boolean);
            infoCard += '<div class="biz-info-row"><div class="biz-info-circle">🏷️</div>'
                      + '<div class="biz-tag-chips">'
                      + tags.map(t => '<span class="biz-tag-chip">' + escapeHtml(t) + '</span>').join('')
                      + '</div></div>';
        }
        if (infoCard) p += '<div class="biz-info-card">' + infoCard + '</div>';

        // ── Rating placeholder (lazy loaded on popupopen) ──
        p += '<div class="biz-rating-row">';
        p += '<div data-rating-id="' + n.id + '" style="min-height:18px;">'
           + '<span style="color:#cbd5e1;font-size:11px;">⭐ cargando reseñas...</span></div>';
        p += '</div>';
        ...
        const bizUrl = window.location.origin + '/business/view_business.php?id=' + n.id;
        const _waTypeLabel = BUSINESS_TYPE_LABELS[n.business_type] || n.business_type || '';
        const _waLines = ['🏪 *' + (n.name || n.nombre || 'Negocio') + '*'];
        if (_waTypeLabel)                    _waLines.push('🗂️ ' + _waTypeLabel);
        if (n.address || n.ubicacion)        _waLines.push('📍 ' + (n.address || n.ubicacion));
        _waLines.push(bizUrl);
        const wa = 'https://wa.me/?text=' + encodeURIComponent(_waLines.join('\n'));

        p += '<div class="popup-footer">';
        if (n.phone) {
            p += '<div class="popup-action-group">'
               + '<a href="tel:' + n.phone + '" class="popup-action" style="background:#27ae60;">📞</a>'
               + '<span class="popup-action-label">Llamar</span>'
               + '</div>';
        }
        if (n.email) {
            p += '<div class="popup-action-group">'
               + '<a href="mailto:' + n.email + '" class="popup-action" style="background:#8e44ad;">📧</a>'
               + '<span class="popup-action-label">Email</span>'
               + '</div>';
        }
        p += '<div class="popup-action-group">'
           + '<a href="' + gmaps + '" target="_blank" class="popup-action" style="background:#4285F4;">🗺️</a>'
           + '<span class="popup-action-label">Mapa</span>'
           + '</div>';
        p += '<div class="popup-action-group">'
           + '<a href="' + wa + '" target="_blank" class="popup-action" style="background:#25D366;">💬</a>'
           + '<span class="popup-action-label">Compartir</span>'
           + '</div>';
        if (n.website) {
            p += '<div class="popup-action-group">'
               + '<a href="' + n.website + '" target="_blank" class="popup-action" style="background:#e67e22;">🌐</a>'
               + '<span class="popup-action-label">Web</span>'
               + '</div>';
        }
        p += '<div class="popup-action-group">'
           + '<a href="/business/view_business.php?id=' + n.id + '" target="_blank" class="popup-action" style="background:#1B3B6F;">📋</a>'
           + '<span class="popup-action-label">Detalle</span>'
           + '</div>';
        // Botón módulo disponibles (solo si el titular activó el módulo)
        if (n.disponibles_activo) {
            p += '<div class="popup-action-group popup-action-group--wide">'
               + '<button type="button" class="popup-action popup-action--text" style="background:#d97706;font-weight:800;" '
               + 'onclick="abrirDisponibles(' + parseInt(n.id) + ',' + toJsSingleStr(n.name || n.nombre || '') + ')">$$$</button>'
               + '<span class="popup-action-label">Stock</span>'
               + '</div>';
        }
        // Botón módulo Busco Empleados/as (solo si hay oferta activa)
        if (n.job_offer_active) {
            p += '<div class="popup-action-group popup-action-group--wide">'
               + '<button type="button" class="popup-action popup-action--text" style="background:#1d4ed8;font-weight:700;" '
               + 'onclick="abrirOfertaTrabajo(' + parseInt(n.id) + ',' + toJsSingleStr(n.name || n.nombre || '') + ')">💼 Empleos</button>'
               + '<span class="popup-action-label">Empleos</span>'
               + '</div>';
        }
        // Botón "Ver Inmuebles" para inmobiliarias
        if (n.business_type === 'inmobiliaria') {
            p += '<div class="popup-action-group popup-action-group--wide">'
               + '<button type="button" class="popup-action popup-action--text" style="background:#16a34a;font-weight:700;color:#fff;" '
               + 'onclick="verInmueblesDe(' + parseInt(n.id) + ',' + toJsSingleStr(n.name || n.nombre || '') + ')">🏘️ Inmuebles</button>'
               + '<span class="popup-action-label">Inmuebles</span>'
               + '</div>';
        }
        p += '</div>';
    }
...
function copyAtSymbol() {
    const status = document.getElementById('copy-at-map-status');
    const btn = document.getElementById('btn-copy-at-map');
    ...
    const finalize = () => {
        ...
        setTimeout(() => {
            if (!status) return;
            status.textContent = 'Atajo util para completar email en celular.';
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText('@')
            .then(() => {
                setStatus('Listo: @ copiado', true);
                finalize();
            })
            .catch(() => {
                setStatus('No se pudo copiar automaticamente', false);
                finalize();
            });
        return;
    }
...
// ─── Location mode (GPS / Virtual) ───────────────────────────────────────────────
function setLocationMode(mode) {
    locationMode = mode;
    try { localStorage.setItem('mapita_location_mode', mode); } catch {}

    const btnGps     = document.getElementById('loc-btn-gps');
    const btnVirt    = document.getElementById('loc-btn-virtual');
    const gpsActions = document.getElementById('loc-gps-actions');
    const virtActions = document.getElementById('loc-virtual-actions');
    const btnFollow  = document.getElementById('btn-follow-me');

    if (btnGps)     btnGps.classList.toggle('loc-mode-btn--active', mode === 'gps');
    if (btnVirt)    btnVirt.classList.toggle('loc-mode-btn--active', mode === 'virtual');
    if (gpsActions) gpsActions.style.display = mode === 'gps'     ? 'flex' : 'none';
    if (virtActions) virtActions.style.display = mode === 'virtual' ? 'block' : 'none';
    if (btnFollow)  btnFollow.style.display = mode === 'gps' ? '' : 'none';

    // Detener seguimiento GPS si se pasa a virtual
    if (mode === 'virtual' && followMe) {
        toggleFollowMe();
    }

    // Redibujar círculo con la ubicación efectiva
    updateRadiusCircle();
    filtrar();
}

function activateVirtualPlacement() {
    _virtualPlacementActive = true;
    const btn = document.getElementById('btn-place-virtual');
    if (btn) { btn.textContent = '🖱️ Hacé clic en el mapa…'; btn.style.background = '#f59e0b'; }
    mapa.once('click', _onMapClickVirtual);
}

function _onMapClickVirtual(e) {
    _virtualPlacementActive = false;
    virtualMarkerPos = { lat: e.latlng.lat, lng: e.latlng.lng };

    // Crear o mover el marcador virtual arrastrable
    if (virtualMarker) {
        virtualMarker.setLatLng(e.latlng);
    } else {
        virtualMarker = L.marker(e.latlng, {
            draggable: true,
            icon: L.divIcon({
                html: '<div style="display:flex;align-items:center;justify-content:center;width:30px;height:30px;background:#0891b2;border:3px solid white;border-radius:50%;box-shadow:0 2px 8px rgba(8,145,178,0.4);"><span style="font-size:14px;">🗺️</span></div>',
                className: '',
                iconSize: [30, 30],
                iconAnchor: [15, 15]
            })
        })
        .bindPopup('🗺️ Punto virtual (arrastrá para mover)')
        .addTo(mapa);

        virtualMarker.on('dragend', ev => {
            virtualMarkerPos = { lat: ev.target.getLatLng().lat, lng: ev.target.getLatLng().lng };
            updateRadiusCircle();
            filtrar();
        });
    }

    const btnPlace = document.getElementById('btn-place-virtual');
    const btnRecenter = document.getElementById('btn-recenter-virtual');
    if (btnPlace) { btnPlace.textContent = '📌 Mover punto virtual'; btnPlace.style.background = '#0891b2'; }
    if (btnRecenter) btnRecenter.style.display = '';
...
function recentrarVirtual() {
    if (virtualMarkerPos && mapa) {
        mapa.setView([virtualMarkerPos.lat, virtualMarkerPos.lng], 14);
    }
}

function updateLocationUIState() {
    const hasLocation = !!getEffectiveUserLocation();
    const clearRow = document.getElementById('loc-clear-row');
    if (clearRow) clearRow.style.display = hasLocation ? '' : 'none';
}
...
function clearLocation() {
    ...
    if (followMe) {
        ...
        const btnFollow = document.getElementById('btn-follow-me');
        if (btnFollow) {
            btnFollow.style.background = '#95a5a6';
            btnFollow.textContent = '📡 Seguirme';
    ...
    if (virtualMarker)      { mapa.removeLayer(virtualMarker);      virtualMarker = null; }
    if (userRadiusCircle)   { mapa.removeLayer(userRadiusCircle);   userRadiusCircle = null; }
    // Limpiar posiciones
    miUbicacion    = null;
    virtualMarkerPos = null;
    // Resetear botones del modo virtual
    const btnPlace    = document.getElementById('btn-place-virtual');
    const btnRecenter = document.getElementById('btn-recenter-virtual');
    if (btnPlace)    { btnPlace.textContent = '📌 Colocar punto virtual'; btnPlace.style.background = '#0891b2'; }
    if (btnRecenter) btnRecenter.style.display = 'none';
    // Actualizar UI y refiltrar
    updateLocationUIState();
    filtrar();
}

// ─── Search preferences ───────────────────────────────────────────────────────────
const LS_KEY_SEARCH_PREFS = 'mapita_search_prefs_v1';

function loadSearchPrefs() {
    try {
        const raw = localStorage.getItem(LS_KEY_SEARCH_PREFS);
        if (!raw) return;
        const p = JSON.parse(raw);
        if (p.maxItems)      searchMaxItems     = p.maxItems;
        if (p.zoomFirst)     searchZoomFirst    = p.zoomFirst;
        if (p.zoomSelected)  searchZoomSelected = p.zoomSelected;

        const selMax  = document.getElementById('pref-max-items');
        const selZ1   = document.getElementById('pref-zoom-first');
        const selZSel = document.getElementById('pref-zoom-selected');
        if (selMax)  selMax.value  = String(searchMaxItems);
        if (selZ1)   selZ1.value   = String(searchZoomFirst);
        if (selZSel) selZSel.value = String(searchZoomSelected);
    } catch {}
}

function saveSearchPrefs() {
    searchMaxItems     = parseInt(document.getElementById('pref-max-items')?.value)    || 10;
    searchZoomFirst    = parseInt(document.getElementById('pref-zoom-first')?.value)   || 15;
    searchZoomSelected = parseInt(document.getElementById('pref-zoom-selected')?.value) || 16;
    try { localStorage.setItem(LS_KEY_SEARCH_PREFS, JSON.stringify({ maxItems: searchMaxItems, zoomFirst: searchZoomFirst, zoomSelected: searchZoomSelected })); } catch {}
}
...
function _restoreInfoPanels() {
    ['filters', 'location', 'results', 'layers', 'tools'].forEach(id => {
        try {
            const val = localStorage.getItem('mapita_info_' + id);
            if (val === '1') {
...
function getLocationFilter() {
    const loc = getEffectiveUserLocation();
    if (!loc) return null;
    const radius = parseInt(document.getElementById('sb-radius-select')?.value) || 5;
    return { enabled: true, radius };
}

function getLocationCityFilter() {
    return null; // legacy — city filter removed
}
...
function updateRadiusCircle() {
    ...
    userRadiusCircle = L.circle([loc.lat, loc.lng], {
        radius: radiusMeters,
        fillColor: '#667eea',
        color: '#667eea',
        weight: 2,
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // ── Restore search preferences from localStorage ───────────────────────────
    loadSearchPrefs();

    // ── Restore info panels state ────────────────────────────────────────────────
    _restoreInfoPanels();

    // ── Restore location mode ───────────────────────────────────────────────────
    try {
        const savedMode = localStorage.getItem('mapita_location_mode');
        if (savedMode === 'virtual') setLocationMode('virtual');
    } catch {}

    // sb-radius-select change → update circle in real-time
    document.getElementById('sb-radius-select')?.addEventListener('change', () => updateRadiusCircle());
...
function abrirEncuesta(encuestaId, titulo) {
    ...
    var prevModal = document.getElementById('encuesta-modal-overlay');
    if (prevModal) prevModal.remove();
    respuestasSeleccionadas = {};

    // Crear overlay
    var modal = document.createElement('div');
    modal.id = 'encuesta-modal-overlay';
    modal.style.cssText = 'position:fixed;top:0;left:0;z-index:5000;width:100%;height:100%;background:rgba(0,0,0,0.72);display:flex;align-items:center;justify-content:center;padding:12px;box-sizing:border-box;';
    ...
    // Cargar encuesta y verificar si ya respondió (en paralelo)
    var urlEnc = '/api/encuestas.php?id=' + encuestaId;
    var urlChk = '/api/encuestas.php?action=check_respondido&id=' + encuestaId;

    var promEnc = fetch(urlEnc).then(function(r){ return r.json(); });
    var promChk = loggedUserId
        ? fetch(urlChk).then(function(r){ return r.json(); })
        : Promise.resolve({success:true, data:{respondio:false}});

    Promise.all([promEnc, promChk])
    .then(function(results){
        var encData   = results[0];
        var chkData   = results[1];
        var yaRespondio = chkData.success && chkData.data && chkData.data.respondio;

        if (!encData.success || !encData.data) {
            content.innerHTML = '<p style="color:#e74c3c;text-align:center;">Error al cargar la encuesta.</p>';
            return;
        }
        var enc = encData.data;
        ...
        if (yaRespondio) {
            ...
            .then(function(res){
                if (res.success && res.data) {
                    encuestaRenderResultados(content, res.data.total_participantes, res.data.preguntas, enc.titulo, true);
                } else {
                    encuestaRenderFormulario(content, enc, encuestaId, loggedUserId);
                }
            })
            .catch(function(){ encuestaRenderFormulario(content, enc, encuestaId, loggedUserId); });
        } else {
            encuestaRenderFormulario(content, enc, encuestaId, loggedUserId);
        }
    })
    .catch(function(e){
        console.error('Error abrirEncuesta', e);
        content.innerHTML = '<p style="color:#e74c3c;text-align:center;">Error al cargar la encuesta.</p>';
    });
}

function encuestaRenderFormulario(content, enc, encuestaId, loggedUserId) {
    var preguntas = enc.preguntas || [];
    ...
    if (total > 0) {
        html += '<div style="font-size:11px;color:#888;margin-bottom:14px;font-weight:600;letter-spacing:0.3px;">'
              + total + ' pregunta' + (total !== 1 ? 's' : '') + '</div>';
    }

    if (preguntas.length > 0) {
        preguntas.forEach(function(preg, idx) {
            html += '<div style="margin-bottom:18px;padding:14px;background:#f8f9fa;border-radius:10px;border-left:3px solid #f39c12;">';
            html += '<p style="margin:0 0 12px;font-weight:700;color:#2c3e50;font-size:14px;">'
                  + (idx+1) + '. ' + preg.pregunta
                  + (preg.requerida ? ' <span style="color:#e74c3c;font-size:11px;">*</span>' : '')
                  + '</p>';

            var pid = preg.id;
            var tipo = preg.tipo || 'opcion_multiple';

            if (tipo === 'si_no') {
                html += '<div style="display:flex;gap:10px;">';
                html += encuestaBotonSiNo(pid, 'Sí', '✅');
                html += encuestaBotonSiNo(pid, 'No', '❌');
                html += '</div>';
                ...
                for (var s = 1; s <= 5; s++) {
                    var op = (6 - s) / 5;
                    html += '<button type="button" data-pid="' + pid + '" data-val="' + s + '"'
                          + ' onclick="encuestaSeleccionar(this,' + pid + ',\'' + s + '\')"'
                          + ' style="flex:1;min-width:40px;height:44px;border:2px solid #f39c12;border-radius:8px;background:#fff;cursor:pointer;font-size:16px;font-weight:700;transition:0.2s;opacity:' + op.toFixed(1) + '">★ ' + s + '</button>';
                }
                html += '</div>';

            } else if (tipo === 'escala_10') {
                var colors = ['#e74c3c','#e74c3c','#e67e22','#e67e22','#f39c12','#f39c12','#f1c40f','#2ecc71','#27ae60','#16a085'];
                html += '<div style="display:flex;gap:4px;flex-wrap:wrap;">';
                for (var s10 = 1; s10 <= 10; s10++) {
                    html += '<button type="button" data-pid="' + pid + '" data-val="' + s10 + '"'
                          + ' onclick="encuestaSeleccionar(this,' + pid + ',\'' + s10 + '\')"'
                          + ' style="width:42px;height:42px;border:2px solid ' + colors[s10-1] + ';border-radius:8px;background:#fff;cursor:pointer;font-size:13px;font-weight:700;transition:0.2s;color:' + colors[s10-1] + '">' + s10 + '</button>';
                }
                html += '</div>';

            } else if (tipo === 'texto') {
                html += '<textarea data-pid="' + pid + '" onchange="respuestasSeleccionadas[' + pid + ']=this.value" oninput="respuestasSeleccionadas[' + pid + ']=this.value"'
                      + ' placeholder="Escribe tu respuesta aquí…"'
                      + ' style="width:100%;padding:10px;border:2px solid #ddd;border-radius:8px;font-family:inherit;font-size:13px;min-height:80px;box-sizing:border-box;resize:vertical;outline:none;transition:0.2s;"'
                      + ' onfocus="this.style.borderColor=\'#f39c12\'" onblur="this.style.borderColor=\'#ddd\'"></textarea>';

            } else {
                // opcion_multiple: tarjetas de botón
                var opciones = preg.opciones_array || [];
                html += '<div style="display:flex;flex-direction:column;gap:6px;">';
                opciones.forEach(function(opt) {
                    html += '<button type="button" data-pid="' + pid + '" data-val="' + opt.replace(/"/g,'&quot;') + '"'
                          + ' onclick="encuestaSeleccionar(this,' + pid + ',this.dataset.val)"'
                          + ' style="width:100%;padding:10px 14px;border:2px solid #e0e0e0;border-radius:8px;background:#fff;cursor:pointer;text-align:left;font-size:13px;font-weight:600;transition:0.2s;color:#2c3e50;"'
                          + ' onmouseover="if(!this.classList.contains(\'enc-sel\'))this.style.borderColor=\'#f39c12\'"'
                          + ' onmouseout="if(!this.classList.contains(\'enc-sel\'))this.style.borderColor=\'#e0e0e0\'">'
                          + opt + '</button>';
                });
                html += '</div>';
            }

            html += '</div>'; // end pregunta block
        });

        html += '<button type="button" onclick="encuestaEnviar(' + encuestaId + ')"'
              + ' style="width:100%;padding:13px;background:#f39c12;color:#fff;border:none;border-radius:10px;font-size:15px;font-weight:700;cursor:pointer;margin-top:6px;transition:0.2s;"'
              + ' onmouseover="this.style.background=\'#d68910\'" onmouseout="this.style.background=\'#f39c12\'">'
              + '✓ Enviar respuestas</button>';
    } else {
        html += '<p style="color:#999;text-align:center;padding:20px 0;">Esta encuesta aún no tiene preguntas.</p>';
    }

    content.innerHTML = html;
}

function encuestaBotonSiNo(pid, valor, emoji) {
    return '<button type="button" data-pid="' + pid + '" data-val="' + valor + '"'
         + ' onclick="encuestaSeleccionar(this,' + pid + ',\'' + valor + '\')"'
         + ' style="flex:1;padding:14px;border:2px solid #ddd;border-radius:10px;background:#fff;cursor:pointer;font-size:15px;font-weight:700;transition:0.2s;">'
         + emoji + ' ' + valor + '</button>';
}

function encuestaSeleccionar(btn, pid, valor) {
    // Deseleccionar otros botones del mismo pid
    var container = btn.closest('[style*="background:#f8f9fa"]') || btn.parentElement;
    container.querySelectorAll('[data-pid="' + pid + '"]').forEach(function(b){
        b.classList.remove('enc-sel');
        b.style.background = '#fff';
        b.style.borderColor = b.dataset.origBorder || '#e0e0e0';
        b.style.color = b.dataset.origColor || '';
        b.style.opacity = b.dataset.origOpacity || '1';
    });
    // Seleccionar el actual
    btn.classList.add('enc-sel');
    btn.style.background = '#f39c12';
    btn.style.borderColor = '#f39c12';
    btn.style.color = '#fff';
    btn.style.opacity = '1';
    respuestasSeleccionadas[pid] = valor;
}
...
function encuestaEnviar(encuestaId) {
    ...
    // Recoger textareas pendientes
    var content = document.getElementById('encuesta-modal-content');
    if (content) {
        content.querySelectorAll('textarea[data-pid]').forEach(function(ta){
            if (ta.value.trim()) respuestasSeleccionadas[ta.dataset.pid] = ta.value.trim();
        });
    }
    ...
    fetch('/api/encuestas.php?action=respond', {
        method: 'POST',
        headers: {'Content-Type':'application/json'},
        body: JSON.stringify({encuesta_id: encuestaId, respuestas: respuestasSeleccionadas})
    })
    .then(function(r){
        // Guardar el status HTTP para detectar 409 antes de parsear JSON
        return r.json().then(function(d){ d._httpStatus = r.status; return d; });
    })
    .then(function(data){
        if (data.success) {
            ...
            .then(function(res){
                var c = document.getElementById('encuesta-modal-content');
                if (c && res.success && res.data) {
                    encuestaRenderResultados(c, res.data.total_participantes, res.data.preguntas, '', false);
                } else if (c) {
                    c.innerHTML = '<div style="text-align:center;padding:30px;">'
                                + '<span style="font-size:40px;display:block;margin-bottom:12px;">🎉</span>'
                                + '<h3 style="color:#27ae60;">¡Gracias por participar!</h3>'
                                + '<button onclick="document.getElementById(\'encuesta-modal-overlay\').remove()" style="margin-top:16px;padding:10px 24px;background:#f39c12;color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:700;">Cerrar</button>'
                                + '</div>';
                }
            })
            .catch(function(){
...
function encuestaRenderResultados(container, totalPart, preguntas, tituloExtra, yaRespondioAntes) {
    var html = '<div style="text-align:center;margin-bottom:20px;">'
             + '<span style="font-size:36px;display:block;margin-bottom:8px;">🎉</span>';
    if (yaRespondioAntes) {
        html += '<h3 style="color:#f39c12;margin:0 0 4px;">Ya respondiste esta encuesta</h3>';
        html += '<p style="color:#888;font-size:13px;margin:0;">Aquí están los resultados actuales</p>';
    } else {
        html += '<h3 style="color:#27ae60;margin:0 0 4px;">¡Gracias por participar!</h3>';
    }
    if (totalPart > 0) {
        html += '<div style="margin-top:8px;display:inline-block;background:#f0f4ff;border-radius:20px;padding:4px 14px;font-size:12px;font-weight:700;color:#1B3B6F;">'
              + totalPart + ' persona' + (totalPart !== 1 ? 's' : '') + ' respondieron</div>';
    }
    html += '</div>';

    if (preguntas && preguntas.length > 0) {
        preguntas.forEach(function(preg) {
            html += '<div style="margin-bottom:18px;padding:14px;background:#f8f9fa;border-radius:10px;">';
            html += '<p style="margin:0 0 10px;font-weight:700;color:#2c3e50;font-size:13px;">'
                  + preg.pregunta
                  + (preg.total > 0 ? ' <span style="font-weight:400;color:#888;font-size:11px;">(' + preg.total + ' resp.)</span>' : '')
                  + '</p>';

            if (preg.datos && preg.datos.length > 0) {
                preg.datos.forEach(function(d) {
                    html += '<div style="margin-bottom:8px;">';
                    html += '<div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:3px;">'
                          + '<span style="font-weight:600;color:#444;">' + d.respuesta + '</span>'
                          + '<span style="font-weight:700;color:#f39c12;">' + d.porcentaje + '%</span></div>';
                    html += '<div style="background:#e9ecef;border-radius:6px;height:10px;overflow:hidden;">'
                          + '<div style="background:linear-gradient(90deg,#f39c12,#e67e22);height:100%;border-radius:6px;width:' + d.porcentaje + '%;transition:width 0.6s ease;"></div>'
                          + '</div>';
                    html += '</div>';
                });
            } else {
                html += '<p style="color:#aaa;font-size:12px;font-style:italic;">Sin respuestas aún</p>';
            }
            html += '</div>';
        });
    }

    html += '<button onclick="document.getElementById(\'encuesta-modal-overlay\').remove()"'
          + ' style="width:100%;padding:12px;background:#1B3B6F;color:#fff;border:none;border-radius:10px;font-weight:700;cursor:pointer;font-size:14px;">Cerrar</button>';
...
function compartirContenido(tipo, id, titulo, localidad) {
    ...
    if (navigator.share) {
        navigator.share({ title: titulo || 'MAPITA', text: texto, url: url })
            .catch(function(){});
        return;
    }

    // Fallback: modal con botones de redes
    ...
    overlay.innerHTML = '<div style="background:#fff;border-radius:16px;padding:28px 24px 22px;max-width:380px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.3);text-align:center;">'
        + '<h3 style="margin:0 0 6px;color:#1B3B6F;font-size:17px;">📤 Compartir</h3>'
        + '<p style="color:#666;font-size:12px;margin:0 0 20px;line-height:1.4;">' + (titulo || '') + '</p>'
        ...
        + '<button onclick="var i=document.getElementById(\'share-link-input\');i.select();document.execCommand(\'copy\');this.textContent=\'✓ Copiado\';setTimeout(()=>this.textContent=\'Copiar\',2000)" style="border:none;background:#1B3B6F;color:#fff;border-radius:6px;padding:5px 10px;cursor:pointer;font-size:11px;font-weight:700;white-space:nowrap;">Copiar</button>'
        + '</div>'
...
function mostrarEventosWidget(eventos) {
    ...
    eventos.forEach(evt => {
        ...
        const titulo = document.createElement('h4');
        titulo.textContent = (evt.icono ? evt.icono + ' ' : '') + evt.titulo;
        titulo.style.cssText = 'margin: 0 0 4px 0; font-size: 13px; color: #e74c3c; font-weight: 600;';

        const info = document.createElement('p');
        let infoText = '';
        if (evt.fecha_inicio || evt.fecha) {
            try {
                const dt = new Date((evt.fecha_inicio || evt.fecha) + ((!evt.fecha_inicio && evt.hora) ? ' ' + evt.hora : ''));
                infoText = dt.toLocaleDateString('es-AR', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
            } catch (e) {
                infoText = (evt.fecha_inicio || evt.fecha || '').substring(0, 10);
            }
        }
        info.textContent = infoText + (evt.ubicacion ? ' · ' + evt.ubicacion : '');
        info.style.cssText = 'margin: 0 0 6px; font-size: 11px; color: #bf360c; line-height: 1.3;';

        const btns = document.createElement('div');
        btns.style.cssText = 'display:flex;gap:5px;flex-wrap:wrap;';
        btns.innerHTML = '<button onclick="event.stopPropagation();compartirContenido(\'evento\',' + evt.id + ',\'' + (evt.titulo||'').replace(/'/g,'') + '\')" style="font-size:10px;padding:3px 8px;background:#1B3B6F;color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:700;">📤 Compartir</button>'
            + (evt.youtube_link ? '<a href="' + evt.youtube_link + '" target="_blank" rel="noopener" onclick="event.stopPropagation()" style="font-size:10px;padding:3px 8px;background:#ff0000;color:#fff;border-radius:8px;text-decoration:none;font-weight:700;">▶ YouTube</a>' : '')
            + (evt.link ? '<a href="' + evt.link + '" target="_blank" rel="noopener" onclick="event.stopPropagation()" style="font-size:10px;padding:3px 8px;background:#27ae60;color:#fff;border-radius:8px;text-decoration:none;font-weight:700;">🔗 Info</a>' : '');
...
function mostrarMarcadoresEventos(eventos) {
    ...
    eventos.forEach(function(evt) {
        ...
        var hasLink = !!evt.link;
        var evColor = hasYT ? '#e74c3c' : '#c0392b';
        var evEmoji = evt.icono || (hasYT ? '▶' : '🎉');
        ...
        var popupHtml = '<div style="font-family:inherit;min-width:220px;background:#fff;border-radius:10px;overflow:hidden;">'
            + '<div style="background:linear-gradient(135deg,#e74c3c,#c0392b);color:white;padding:14px 16px;margin-bottom:10px;">'
            + '<strong style="font-size:15px;font-weight:800;display:block;line-height:1.25;letter-spacing:-.01em;">' + escapeHtml(evt.titulo) + '</strong>'
            + (evt.categoria ? '<span style="font-size:10px;font-weight:600;opacity:.85;display:block;margin-top:3px;letter-spacing:.04em;text-transform:uppercase;">' + escapeHtml(evt.categoria) + '</span>' : '')
            + '</div>'
        ...
        if (hasYT) popupHtml += '<a href="' + evt.youtube_link + '" target="_blank" rel="noopener" style="padding:6px 12px;background:#ff0000;color:white;border:none;border-radius:12px;cursor:pointer;font-size:12px;text-decoration:none;display:inline-block;">▶ YouTube</a>';
        if (hasLink) popupHtml += '<a href="' + evt.link + '" target="_blank" rel="noopener" style="padding:6px 12px;background:#27ae60;color:white;border:none;border-radius:12px;cursor:pointer;font-size:12px;text-decoration:none;display:inline-block;">🔗 Más info</a>';
        ...
        popupHtml += '<button onclick="compartirContenido(\'evento\',' + evt.id + ',\'' + evt.titulo.replace(/'/g,"'") + '\')" style="padding:6px 12px;background:#1B3B6F;color:white;border:none;border-radius:12px;cursor:pointer;font-size:12px;">📤 Compartir</button>';
...
function abrirNoticiaModal(noticiaId, titulo, noticiaData) {
    ...
    if (noticiaData.youtube_link && noticiaData.youtube_link.trim()) {
        const ytDiv = document.createElement('div');
        ytDiv.style.cssText = 'margin-bottom:14px;';
        const ytId = noticiaData.youtube_link.match(/(?:v=|youtu\.be\/)([A-Za-z0-9_-]{11})/);
        if (ytId) {
            ytDiv.innerHTML = `<iframe width="100%" height="200" src="https://www.youtube.com/embed/${ytId[1]}" frameborder="0" allow="accelerometer;autoplay;encrypted-media;gyroscope;picture-in-picture" allowfullscreen style="border-radius:8px;"></iframe>`;
        } else {
            ytDiv.innerHTML = `<a href="${noticiaData.youtube_link.trim()}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:6px;padding:8px 14px;background:#ff0000;color:white;border-radius:8px;text-decoration:none;font-weight:600;font-size:13px;">▶ Ver en YouTube</a>`;
        }
    ...
    footer.style.cssText = 'display: flex; gap: 8px; padding-top: 16px; border-top: 1px solid #eee; flex-wrap:wrap;';
    const _leerBtn = (noticiaData.link && noticiaData.link.trim())
        ? `<a href="${noticiaData.link.trim()}" target="_blank" rel="noopener noreferrer" style="flex:1;min-width:120px;padding:10px;background:linear-gradient(135deg,#667eea,#764ba2);color:white;text-decoration:none;text-align:center;border-radius:6px;font-weight:600;font-size:13px;">🔗 Leer Completo</a>`
        : '';
    const _noticiaId = noticiaId;
    const _noticiaTit = (noticiaData.titulo || '').replace(/'/g, '');
    footer.innerHTML = `
        ${_leerBtn}
        <button onclick="compartirContenido('noticia',${_noticiaId},'${_noticiaTit}')" style="flex:1;min-width:100px;padding:10px;background:#1B3B6F;color:white;border:none;border-radius:6px;cursor:pointer;font-weight:600;font-size:13px;">📤 Compartir</button>
        <button onclick="this.closest('div').parentElement.parentElement.remove()" style="flex:1;min-width:80px;padding:10px;background:#f0f0f0;border:none;border-radius:6px;cursor:pointer;font-weight:600;font-size:13px;">Cerrar</button>
    `;
...
function mostrarMarcadoresNoticias(noticias) {
    ...
    noticias.forEach(noticia => {
        ...
        const notTituloSafe = (noticia.titulo || '').replace(/'/g, '').replace(/"/g, '');
        let popup = '<div style="font-family:inherit;min-width:220px;background:#fff;border-radius:10px;overflow:hidden;">'
            + '<div style="background:linear-gradient(135deg,#667eea,#764ba2);color:white;padding:14px 14px 12px;">'
            + '<strong style="font-size:14px;line-height:1.3;display:block;">📰 ' + noticia.titulo + '</strong></div>'
            + '<div style="padding:10px 12px 4px">';
        if (noticia.categoria) popup += '<p style="margin:0 0 6px;font-size:11px;background:#f0f2ff;display:inline-block;padding:2px 8px;border-radius:10px;color:#667eea;font-weight:600">' + noticia.categoria + '</p>';
        if (textoPrev) popup += '<p style="margin:6px 0;font-size:12px;color:#555;line-height:1.4">' + textoPrev + '…</p>';
        ...
        if (noticia.ubicacion) popup += '<p style="margin:4px 0;font-size:12px;color:#555;">📍 ' + noticia.ubicacion + '</p>';
        popup += '<button onclick="abrirNoticiaPorId(' + noticia.id + ')" style="width:100%;padding:9px 8px;background:#667eea;color:white;border:none;border-radius:8px;cursor:pointer;font-size:13px;font-weight:700;margin-top:6px;margin-bottom:6px;">📰 Leer más</button>';
        popup += '<button onclick="compartirContenido(\'noticia\',' + noticia.id + ',\'' + notTituloSafe + '\')" style="width:100%;padding:7px 8px;background:#fff;color:#667eea;border:1.5px solid #667eea;border-radius:8px;cursor:pointer;font-size:12px;font-weight:600;margin-bottom:6px;">📤 Compartir noticia</button>';
        popup += '</div></div>';
...
function _buildInmPopup(inm, overrideBizId, overrideBizName) {
    ...
    const hasPhoto   = inm.foto_url && (/^https?:\/\//i.test(inm.foto_url) || /^\//.test(inm.foto_url));
    const badgeCls   = inm.operacion === 'alquiler' ? 'inm-v2-badge--alquiler' : 'inm-v2-badge--venta';

    let p = '<div class="inm-v2">';

    // ── HERO — full-width image (desktop only; hidden on mobile via CSS) ──────
    p += '<div class="inm-v2-hero">';
    if (hasPhoto) {
        p += '<img class="inm-v2-hero-img" src="' + escapeHtml(inm.foto_url) + '" alt="Foto del inmueble" loading="eager"'
           + ' onerror="this.style.display=\'none\'">';
    } else {
        p += '<div class="inm-v2-hero-placeholder">' + tipoIcon + '</div>';
    }
    p += '<div class="inm-v2-hero-overlay"></div>';
    p += '<div class="inm-v2-badge ' + badgeCls + '">' + opEmoji + ' ' + escapeHtml(opLabel) + '</div>';
    p += '<div class="inm-v2-hero-info">';
    p += '<div class="inm-v2-hero-title">' + escapeHtml(inm.titulo || 'Inmueble') + '</div>';
    if (precio) {
        p += '<div class="inm-v2-hero-price">' + escapeHtml(precio) + '</div>';
    }
    p += '</div>';
    p += '</div>'; // inm-v2-hero

    // ── MOBILE HEADER — green gradient (shown only on ≤ 480 px) ──────────────
    p += '<div class="inm-v2-mob-hdr">';
    p += '<div class="inm-v2-mob-hdr-icon">' + tipoIcon + '</div>';
    p += '<div class="inm-v2-mob-hdr-text">';
    p += '<div class="inm-v2-mob-title">' + escapeHtml(inm.titulo || 'Inmueble') + '</div>';
    if (precio) {
        p += '<div class="inm-v2-mob-price">' + escapeHtml(opLabel + ' — ' + precio) + '</div>';
    } else {
        p += '<div class="inm-v2-mob-price">' + opEmoji + ' ' + escapeHtml(opLabel) + '</div>';
    }
    p += '</div></div>'; // .inm-v2-mob-hdr

    // ── BODY ─────────────────────────────────────────────────────────────────
    p += '<div class="inm-v2-body">';

    // Feature chips: tipo, ambientes, superficie, financiado
    const chips = [];
    ...
    if (chips.length) {
        p += '<div class="inm-v2-chips">';
        chips.forEach(c => {
            p += '<span class="inm-v2-chip' + (c.fin ? ' inm-v2-chip--fin' : '') + '">' + escapeHtml(c.text) + '</span>';
        });
        p += '</div>';
    }
    ...
    if (inm.descripcion && inm.descripcion.trim()) {
        const desc = inm.descripcion.length > 90 ? inm.descripcion.substring(0, 90) + '…' : inm.descripcion;
        p += '<p class="inm-v2-desc">' + escapeHtml(desc) + '</p>';
    }

    // Address
    if (inm.direccion) {
        p += '<div class="inm-v2-address">';
        p += '<span class="inm-v2-address-label">📍 Dirección</span>';
        p += '<div class="inm-v2-address-val">' + escapeHtml(inm.direccion) + '</div>';
        p += '</div>';
    }

    // Inmobiliaria / Agent card
    if (inmId) {
        p += '<div class="inm-v2-agent">';
        p += '<div class="inm-v2-agent-avatar">';
        if (inmIcon && /^https?:\/\//i.test(inmIcon)) {
            p += '<img src="' + escapeHtml(inmIcon) + '" alt="' + escapeHtml(inmNombre) + '" onerror="this.style.display=\'none\'">';
        } else {
            p += '🏢';
        }
        p += '</div>';
        p += '<div class="inm-v2-agent-info">';
        p += '<div class="inm-v2-agent-role">Inmobiliaria</div>';
        p += '<div class="inm-v2-agent-name">' + escapeHtml(inmNombre) + '</div>';
        p += '</div>';
        p += '</div>';
    }
    ...
    // ── ACTION BAR ───────────────────────────────────────────────────────────
    p += '<div class="inm-v2-actions">';

    // Ver detalle
    if (inmIdInt) {
        p += '<div class="inm-v2-act-grp">'
           + '<button type="button" class="inm-v2-btn inm-v2-btn--detail" title="Ver detalle del inmueble"'
           + ' onclick="abrirDetalleInmueble(' + inmIdInt + ')">🔍<span class="inm-v2-btn-text"> Detalle</span></button>'
           + '<span class="inm-v2-act-lbl">Detalle</span>'
           + '</div>';
    }

    // Llamar — PRIMARY CTA (wider, green)
    if (inm.contacto) {
        p += '<div class="inm-v2-act-grp">'
           + '<a href="tel:' + escapeHtml(inm.contacto) + '" class="inm-v2-btn inm-v2-btn--call" title="Llamar">📞<span class="inm-v2-btn-text"> Llamar</span></a>'
           + '<span class="inm-v2-act-lbl">Llamar</span>'
           + '</div>';
    }

    // Ver otros inmuebles + Enfocar en mapa
    if (inmId) {
        ...
        p += '<div class="inm-v2-act-grp">'
           + '<button type="button" class="inm-v2-btn inm-v2-btn--list" title="Ver más inmuebles de esta inmobiliaria"'
           + ' onclick="verInmueblesDe(' + safeId2 + ',' + safeNameS + ')">🏘️</button>'
           + '<span class="inm-v2-act-lbl">Ver más</span>'
           + '</div>';
        p += '<div class="inm-v2-act-grp">'
           + '<button type="button" class="inm-v2-btn inm-v2-btn--map" title="Ver inmobiliaria en el mapa"'
           + ' onclick="enfocarInmobiliaria(' + safeId2 + ',' + safeLatF + ',' + safeLngF + ')">🗺️</button>'
           + '<span class="inm-v2-act-lbl">Mapa</span>'
           + '</div>';
    }

    p += '</div>'; // .inm-v2-actions
    p += '</div>'; // .inm-v2
    return p;
}
...
async function _cargarInmueblesCerca() {
    ...
    try {
        ...
        if (_inmFiltroOp) {
            if (_inmFiltroOp === 'proyecto') {
                rows = rows.filter(function(r) { return (r.tipo || '').toLowerCase() === 'proyecto'; });
            } else {
                rows = rows.filter(function(r) { return (r.operacion || '').toLowerCase() === _inmFiltroOp; });
            }
        }
        if (_inmFiltroMin > 0) rows = rows.filter(function(r) { return parseFloat(r.precio) >= _inmFiltroMin; });
        if (_inmFiltroMax > 0) rows = rows.filter(function(r) { return parseFloat(r.precio) <= _inmFiltroMax; });
        // Mostrar contador
        var countEl = document.getElementById('inm-f-count');
        if (countEl) {
            var hasFilter = _inmFiltroOp || _inmFiltroMin > 0 || _inmFiltroMax > 0;
            if (hasFilter) {
                countEl.style.display = 'block';
                countEl.textContent = rows.length + ' de ' + rowsTotal + ' inmuebles';
            } else {
        ...
        rows.forEach(function(inm) {
            ...
            mk.bindPopup(_buildInmPopup(inm, null, null), { maxWidth: 360, className: 'inm-v2-popup' });
...
/* ── Filtros CERCA ─────────────────────────────────────────────────────── */
var _inmFiltroOp  = '';   // '' | 'venta' | 'alquiler' | 'proyecto'
var _inmFiltroMin = 0;
var _inmFiltroMax = 0;

function toggleInmFiltros() {
    var body  = document.getElementById('inm-f-body');
    var arrow = document.getElementById('inm-f-arrow');
    if (!body) return;
...
function setInmOp(el, val) {
    _inmFiltroOp = val;
    document.querySelectorAll('#inm-f-op-grp .inm-f-btn').forEach(function(b) {
        b.classList.toggle('inm-f-btn--on', b === el);
    });
}

function setInmPreset(min, max) {
    document.getElementById('inm-f-pmin').value = min > 0 ? min : '';
    document.getElementById('inm-f-pmax').value = max > 0 ? max : '';
}

function aplicarInmFiltros() {
    _inmFiltroMin = parseInt(document.getElementById('inm-f-pmin').value) || 0;
    _inmFiltroMax = parseInt(document.getElementById('inm-f-pmax').value) || 0;
    if (_cercaActivo) {
        _cargarInmueblesCerca();
    } else {
        activarCerca();
    }
}
...
async function verInmueblesDe(bizId, bizName) {
    ...
    try {
        ...
        d.data.forEach(function(inm) {
            ...
            mk.bindPopup(_buildInmPopup(inm, bizId, bizName), { maxWidth: 360, className: 'inm-v2-popup' });
...
function _showVerInmBanner(bizName, count) {
    ...
    const label = count === 1 ? 'inmueble' : 'inmuebles';
    banner.innerHTML =
        '<div class="vib-icon">🏠</div>' +
        '<div class="vib-body">' +
            '<div class="vib-role">Inmobiliaria registrada</div>' +
            '<div class="vib-name">' + escapeHtml(bizName) + '</div>' +
            '<div class="vib-count">' +
                '<span class="vib-count-badge">' + count + '</span>' +
                '<span class="vib-count-label">' + label + ' en sistema</span>' +
            '</div>' +
        '</div>' +
        '<button class="vib-close" onclick="cerrarVerInmuebles()" aria-label="Cerrar">✕</button>';
...
function abrirDetalleInmueble(inmId) {
    ...
    .then(d => {
        ...
        chars.push({ icon: opEmoji, label: 'Operación', val: opLabel });
        ...
        if (inm.moneda)        chars.push({ icon: '💱',      label: 'Moneda',     val: inm.moneda });
        ...
        if (inm.created_at) {
            try {
                const pubDate = new Date(inm.created_at).toLocaleDateString('es-AR', { day: 'numeric', month: 'long', year: 'numeric' });
                html += '<div class="inm-detail-section">';
                html += '<span class="inm-detail-section__label">📅 Publicado</span>';
                html += '<p class="inm-detail-section__text">' + escapeHtml(pubDate) + '</p>';
                html += '</div>';
            } catch (e) {}
        }

        // Sitio web del inmueble (en el cuerpo, más visible que solo el footer)
        if (inm.web_url) {
            html += '<div class="inm-detail-section">';
            html += '<span class="inm-detail-section__label">🔗 Sitio web del inmueble</span>';
            html += '<a class="inm-detail-section__text" href="' + escapeHtml(inm.web_url) + '" target="_blank" rel="noopener noreferrer"'
                  + ' style="color:#2563eb;word-break:break-all;">' + escapeHtml(inm.web_url) + '</a>';
            html += '</div>';
        }
        ...
        html += '<span class="inm-detail-biz__badge">✓ Registrada en MAPITA</span>';
        html += '</div>';
        html += '</div>';
        ...
        if (bizId2) {
            ...
            ftHtml += '<button type="button" class="inm-detail-btn inm-detail-btn--list"'
                   + ' onclick="cerrarDetalleInmueble();verInmueblesDe(' + bizId2 + ',' + safeNameS + ')">🏘️ Más inmuebles</button>';
        }
...
<div id="mapita-map-watermark" style="
    position:fixed;bottom:76px;right:10px;
    z-index:1001;
...
<div id="mapita-home-panel" role="dialog" aria-modal="false" aria-label="Panel de Inicio">
    <div class="mapita-home-panel__header">
        <div style="flex:1;min-width:0;">
            <a class="home-contact-card" href="mailto:soy@mapita.com.ar" title="soy@mapita.com.ar">✉️ Contacto</a>
...
<!-- ══════════════════════════════════════════════════════════════════════════
     WELCOME OVERLAY — Selección de localidad al iniciar el mapa
     + Toast motivacional "¡ Puedes ser el primero en situar tu negocio aquí !"
     Se muestra solo la primera visita (localStorage: mapita_welcomed_v1)
══════════════════════════════════════════════════════════════════════════ -->

<!-- ════════ WELCOME OVERLAY ════════ -->
<div id="mapita-welcome-overlay" role="dialog" aria-modal="true" aria-label="Elegir localidad">
  <div id="mapita-welcome-card">
    <button id="mapita-welcome-close" title="Cerrar" aria-label="Cerrar panel">✕</button>
    <div id="mapita-welcome-logo">🗺️</div>
    <h2 id="mapita-welcome-title">
      ¿Dónde querés explorar?
      <span class="welcome-lang-en">Where do you want to explore?</span>
    </h2>
    <p id="mapita-welcome-sub">
      Escribí tu localidad o ciudad para empezar
      <span class="welcome-lang-en">Type your city or locality to get started</span>
    </p>
    <div id="mapita-welcome-region-wrap">
      <label for="mapita-welcome-region" id="mapita-welcome-region-label">🌍 Región · Region</label>
      <select id="mapita-welcome-region">
        <option value="sudamerica" selected>🌎 América del Sur</option>
        <option value="centroamerica">🌎 América Central y México</option>
        <option value="norteamerica">🌎 América del Norte</option>
        <option value="europa">🌍 Europa</option>
        <option value="asia">🌏 Asia</option>
        <option value="africa">🌍 África</option>
        <option value="oceania">🌏 Oceanía</option>
        <option value="mundo">🌐 Todo el mundo · Worldwide</option>
      </select>
    </div>
    <div id="mapita-welcome-input-wrap">
      <input
        id="mapita-welcome-input"
        type="text"
        placeholder="Ej: Buenos Aires, Rosario, Mendoza…"
        autocomplete="off"
        maxlength="100"
      />
      <div id="mapita-welcome-suggestions" role="listbox" aria-label="Sugerencias"></div>
    </div>
    <button id="mapita-welcome-btn" disabled>Explorar · Explore →</button>
    <div class="welcome-or-divider"><span>o · or</span></div>
    <button id="mapita-welcome-gps-btn" type="button">📍 Mi ubicación · My location</button>
    <p id="mapita-welcome-gps-error" class="hidden"></p>
    <p id="mapita-welcome-skip">
      <a href="#" id="mapita-welcome-skip-link">continuar con Buenos Aires · continue with Buenos Aires</a>
    </p>
  </div>
</div>

<!-- ════════ TOAST MOTIVACIONAL ════════ -->
<div id="mapita-first-toast" class="hidden" role="status" aria-live="polite">
  <span id="mapita-toast-icon">📍</span>
  <span id="mapita-toast-text">¡ Podés ser el primero en situar tu <strong class="toast-highlight">Negocio</strong> o <strong class="toast-highlight">Marca</strong> aquí !</span>
  <button id="mapita-toast-close" aria-label="Cerrar">✕</button>
</div>
...
<script>
/* ══════════════════════════════════════════════
   WELCOME OVERLAY — localidad + toast motivacional
   Muestra solo 1 vez (localStorage key: mapita_welcomed_v1)
══════════════════════════════════════════════ */
(function () {
    'use strict';
    var STORAGE_KEY = 'mapita_welcomed_v1';

    var overlay      = document.getElementById('mapita-welcome-overlay');
    var card         = document.getElementById('mapita-welcome-card');
    var closeBtn     = document.getElementById('mapita-welcome-close');
    var input        = document.getElementById('mapita-welcome-input');
    var suggestions  = document.getElementById('mapita-welcome-suggestions');
    var exploreBtn   = document.getElementById('mapita-welcome-btn');
    var regionSelect = document.getElementById('mapita-welcome-region');
    var gpsBtn       = document.getElementById('mapita-welcome-gps-btn');
    var gpsError     = document.getElementById('mapita-welcome-gps-error');
    var skipLink     = document.getElementById('mapita-welcome-skip-link');
    var toast        = document.getElementById('mapita-first-toast');
    var toastClose   = document.getElementById('mapita-toast-close');

    /* ── Mapa de regiones → countrycodes + placeholder ─────────────── */
    var REGIONS = {
        sudamerica:    { codes: 'ar,bo,br,cl,co,ec,gy,pe,py,sr,uy,ve',                          ph: 'Ej: Buenos Aires, Lima, Santiago…' },
        centroamerica: { codes: 'cr,cu,do,gt,hn,mx,ni,pa,sv,bz,ht,jm,tt',                       ph: 'Ej: Ciudad de México, San José, Panamá…' },
        norteamerica:  { codes: 'us,ca,mx',                                                       ph: 'Ej: New York, Toronto, Los Angeles…' },
        europa:        { codes: 'es,pt,fr,de,it,gb,nl,be,ch,at,pl,se,no,dk,fi,gr,tr,ro,cz,hu,sk,hr,rs,bg,ua,is,lu,li,mc,ad,sm,mt,cy', ph: 'Ej: Madrid, París, Roma, Berlín…' },
        asia:          { codes: 'cn,jp,kr,in,id,th,vn,ph,my,sg,bd,pk,ir,sa,ae,il,kz,mn,np,lk,mm,kh,la', ph: 'Ej: Tokio, Mumbai, Shanghái, Dubái…' },
        africa:        { codes: 'za,ng,ke,et,eg,dz,ma,tn,gh,tz,ug,rw,mz,zm,zw,bw,na,sn,ci,cm', ph: 'Ej: Ciudad del Cabo, Nairobi, El Cairo…' },
        oceania:       { codes: 'au,nz,pg,fj,sb,vu,ws,to',                                       ph: 'Ej: Sydney, Auckland, Melbourne…' },
        mundo:         { codes: '',                                                                ph: 'Ej: Rosario, París, Tokyo, Sydney…' }
    };

    function currentRegion() {
        return REGIONS[regionSelect.value] || REGIONS.sudamerica;
    }

    /* Actualizar placeholder al cambiar región */
    regionSelect.addEventListener('change', function () {
        input.placeholder = currentRegion().ph;
        /* Limpiar selección previa si el usuario cambia de región */
        input.value = '';
        selectedLat = null;
        selectedLng = null;
    ...
    // Toast ya empieza con class="hidden" en el HTML — no hace falta agregarlo acá

    var selectedLat  = null;
    var selectedLng  = null;
    var debounceTimer = null;

    /* ── openOverlay(): muestra el panel limpio (desde FAB o auto-show) */
    function openOverlay() {
        input.value = '';
        selectedLat = null;
        selectedLng = null;
        exploreBtn.disabled = true;
        regionSelect.value = 'sudamerica';
        input.placeholder = REGIONS.sudamerica.ph;
        closeSuggestions();
        gpsError.classList.add('hidden');
        gpsBtn.classList.remove('loading');
        gpsBtn.disabled = false;
        gpsBtn.textContent = '📍 Mi ubicación · My location';
        overlay.classList.remove('hidden');
        setTimeout(function () { input.focus(); }, 120);
    }

    /* ── FAB: siempre disponible en el mapa para reabrir el panel ──── */
    var fab = document.getElementById('mapita-locality-fab');
    if (fab) {
        fab.addEventListener('click', openOverlay);
    }

    /* ── Auto-show: solo en la primera visita ───────────────────────── */
    if (localStorage.getItem(STORAGE_KEY)) {
        overlay.classList.add('hidden');   /* ya visitó → panel oculto */
    }
    /* Si NO tiene la clave el overlay ya está visible (sin clase hidden) */

    /* ── Nominatim autocomplete (400 ms debounce) ──────────────────── */
    input.addEventListener('input', function () {
        ...
        debounceTimer = setTimeout(function () {
            ...
            var url = 'https://nominatim.openstreetmap.org/search'
                    ...
                    + (region.codes ? '&countrycodes=' + region.codes : '')
    ...
    function renderSuggestions(data) {
        ...
        if (!data || !data.length) { closeSuggestions(); return; }
        data.forEach(function (item) {
            var div = document.createElement('div');
            div.className = 'welcome-suggestion-item';
    ...
    /* Cerrar sugerencias al hacer click fuera de la card */
    document.addEventListener('mousedown', function (e) {
        if (card && !card.contains(e.target)) closeSuggestions();
    });

    /* ── Explorar: navegar al lugar elegido ────────────────────────── */
    exploreBtn.addEventListener('click', function () {
        if (selectedLat === null) return;
        dismissOverlay();
        /* mapa es la variable global de Leaflet definida en inicializarMapa() */
        if (typeof mapa !== 'undefined' && mapa) {
            mapa.setView([selectedLat, selectedLng], 11);
        }
        showToast();
    });

    /* ── GPS: ir a mi ubicación (zoom amplio = sector/ciudad) ──────── */
    if (!navigator.geolocation) {
        /* Navegador sin soporte → ocultar el botón directamente */
        gpsBtn.style.display = 'none';
    } else {
        gpsBtn.addEventListener('click', function () {
            gpsBtn.classList.add('loading');
            gpsBtn.disabled = true;
            gpsBtn.textContent = '⏳ Detectando ubicación…';
            gpsError.classList.add('hidden');

            navigator.geolocation.getCurrentPosition(
                function (pos) {
                    /* Éxito: zoom 12 → muestra ~20 km alrededor, no el pin exacto */
                ...
                function (err) {
                    ...
                    gpsBtn.classList.remove('loading');
                    gpsBtn.disabled = false;
                    gpsBtn.textContent = '📍 Ir a mi ubicación';
                    var msgs = {
                        1: 'Permiso denegado. Habilitá la ubicación en tu navegador.',
                        2: 'No se pudo detectar tu ubicación. Intentá de nuevo.',
                        3: 'Tiempo de espera agotado. Intentá de nuevo.'
                    };
                    gpsError.textContent = msgs[err.code] || 'Error al obtener ubicación.';
    ...
    /* ── Cerrar sin elegir (botón ✕) ───────────────────────────────── */
    closeBtn.addEventListener('click', function () {
        dismissOverlay();
        /* No toast si cerró sin elegir */
    });
    ...
    skipLink.addEventListener('click', function (e) {
        ...
        dismissOverlay();
        if (typeof mapa !== 'undefined' && mapa) {
            mapa.setView([-34.6037, -58.3816], 13);
        }
        showToast();
    });
    ...
    input.addEventListener('keydown', function (e) {
        if (e.key === 'Enter' && !exploreBtn.disabled) { exploreBtn.click(); }
    ...
    function dismissOverlay() {
        ...
        /* Lanzar onboarding tips después de que el panel cierra */
        setTimeout(function () {
            if (typeof window.startOnboardingTips === 'function') window.startOnboardingTips();
        }, 700);
    }

    /* ── Toast motivacional (9 s auto-dismiss) ─────────────────────── */
    function showToast() {
        toast.classList.remove('hidden');
        toast.classList.add('toast-visible');   /* dispara la animación */
        var autoHide = setTimeout(hideToast, 9000);
...
<!-- ══════════════════════════════════════════════════════════════════════════
     ONBOARDING TIPS — Avisos sutiles al primer uso
     Apuntan a los elementos clave del mapa, uno a la vez
     Key localStorage: mapita_tips_v1
══════════════════════════════════════════════════════════════════════════ -->
<div id="mapita-onboarding-tip" class="hidden" role="tooltip">
  <button id="mapita-tip-dismiss" aria-label="Cerrar sugerencias" title="No mostrar más">✕</button>
  <div id="mapita-tip-emoji"></div>
  <p id="mapita-tip-text"></p>
  <div class="mapita-tip-footer">
    <span id="mapita-tip-counter"></span>
    <button id="mapita-tip-next"></button>
  </div>
</div>
...
<script>
/* ══════════════════════════════════════════════
   ONBOARDING TIPS — primer uso del mapa
   Muestra burbujas sutiles apuntando a los elementos clave
   Se muestra una sola vez (localStorage: mapita_tips_v1)
══════════════════════════════════════════════ */
(function () {
    'use strict';
    var TIPS_KEY = 'mapita_tips_v1';

    var TIPS = [
        {
            targetId: 'busqueda',
            emoji: '🔍',
            text: 'Buscá cualquier negocio o marca por nombre directamente desde aquí',
            skipOnMobile: false
        },
        {
            targetId: 'tipo',
            emoji: '📂',
            text: 'Filtrá por tipo de negocio: restaurante, farmacia, inmobiliaria, marca y muchos más',
            skipOnMobile: false
        },
        {
            targetId: 'filter-open-now',
            emoji: '🟢',
            text: 'Activá este filtro para ver solo los negocios abiertos en este momento',
            skipOnMobile: false
        },
        {
            targetId: 'sidebar-auth-section',
            emoji: '📝',
            text: 'Registrate o iniciá sesión para poner tu negocio o marca en el mapa',
            skipIfLoggedIn: true,
            skipOnMobile: false
        },
        {
            targetId: 'btn-consulta-envio',
            emoji: '🚚',
            text: 'Consultá todos los transportistas disponibles dentro de un radio que vos elegís en el mapa — ideal para coordinar envíos locales'
        },
        {
            targetId: 'btn-convocar',
            emoji: '🎭',
            text: 'Si tenés una Obra de Arte registrada, podés convocar artistas y servicios: MAPITA notifica automáticamente a todos los que coincidan con los roles de tu proyecto'
        },
        {
            targetId: 'mapita-locality-fab',
            emoji: '🌍',
            text: 'Desde aquí podés cambiar la ciudad o zona que explorás en cualquier momento'
        },
        {
            targetId: 'mapita-map-watermark',
            emoji: '💡',
            text: 'Panel de inicio: explorar el mapa, contactar negocios, publicar ofertas, crear canales WT y registrar tu marca'
        },
        {
            targetId: 'map-legend-btn',
            emoji: '🗺️',
            text: 'Consultá qué representa cada ícono y color que aparece en el mapa'
        }
    ];

    var currentIdx       = 0;
    var highlightedEl    = null;   /* elemento resaltado actualmente */
    var bubble           = document.getElementById('mapita-onboarding-tip');
    var emojiEl     = document.getElementById('mapita-tip-emoji');
    var textEl      = document.getElementById('mapita-tip-text');
    var counterEl   = document.getElementById('mapita-tip-counter');
    var nextBtn     = document.getElementById('mapita-tip-next');
    var dismissBtn  = document.getElementById('mapita-tip-dismiss');

    /* Detectar si el usuario está logueado (SESSION_USER_ID definido en map.php) */
    var isLoggedIn = (typeof SESSION_USER_ID !== 'undefined' && SESSION_USER_ID > 0);

    function showTip(index) {
        if (index >= TIPS.length) { endTips(); return; }

        var tip = TIPS[index];

        /* Saltar si es solo para no logueados y el usuario ya tiene sesión */
        if (tip.skipIfLoggedIn && isLoggedIn) { showTip(index + 1); return; }

        var target = document.getElementById(tip.targetId);
        /* Saltar si el elemento no existe o no está visible */
        if (!target || target.offsetParent === null) { showTip(index + 1); return; }

        /* Actualizar contenido */
        emojiEl.textContent   = tip.emoji;
        textEl.textContent    = tip.text;
        /* Contar solo los tips que SÍ se muestran al usuario actual */
        var visibleTotal = TIPS.filter(function(t) {
            return !(t.skipIfLoggedIn && isLoggedIn);
        }).length;
        counterEl.textContent = 'Consejo ' + (index + 1) + ' de ' + visibleTotal;
        nextBtn.textContent   = index < TIPS.length - 1 ? 'Siguiente →' : 'Entendido ✓';

        /* Quitar resaltado del elemento anterior y aplicarlo al actual */
        if (highlightedEl) highlightedEl.classList.remove('tip-target-highlight');
        target.classList.add('tip-target-highlight');
        highlightedEl = target;

        /* Resetear clases de flecha y animación */
        bubble.classList.remove('hidden', 'tip-show', 'tip-arrow-right', 'tip-arrow-left', 'tip-arrow-down');
        void bubble.offsetWidth; /* reflow para re-disparar animación */

        var rect = target.getBoundingClientRect();
        var bW   = bubble.offsetWidth  || 220;
        var bH   = bubble.offsetHeight || 110;
        var gap  = 14;
        var left, top;
        var screenMid = window.innerWidth / 2;

        if (rect.left + rect.width / 2 < screenMid) {
            /* Elemento en lado IZQUIERDO (sidebar) → tip a la DERECHA, flecha izquierda */
            left = rect.right + gap;
            top  = rect.top + (rect.height / 2) - (bH / 2);
            /* Si se sale por la derecha, poner debajo */
            if (left + bW > window.innerWidth - 8) {
                left = Math.max(8, rect.left);
                top  = rect.bottom + gap;
                bubble.classList.add('tip-arrow-down');
            } else {
                bubble.classList.add('tip-arrow-left');
            }
        } else {
            /* Elemento en lado DERECHO (FABs, leyenda) → tip a la IZQUIERDA, flecha derecha */
            left = rect.left - bW - gap;
            top  = rect.top + (rect.height / 2) - (bH / 2);
            /* Si se sale por la izquierda, poner encima */
            if (left < 8) {
                left = Math.max(8, rect.left + (rect.width / 2) - (bW / 2));
                top  = rect.top - bH - gap;
                bubble.classList.add('tip-arrow-down');
            } else {
                bubble.classList.add('tip-arrow-right');
            }
        }
    ...
    function endTips() {
        if (highlightedEl) {
            highlightedEl.classList.remove('tip-target-highlight');
            highlightedEl = null;
        }
    ...
    nextBtn.addEventListener('click', function () {
        showTip(currentIdx + 1);
    });

    /* Cerrar todos */
    dismissBtn.addEventListener('click', endTips);

    /* ── API pública — llamada desde dismissOverlay() del welcome panel ── */
    window.startOnboardingTips = function () {
        if (localStorage.getItem(TIPS_KEY)) return;
        setTimeout(function () { showTip(0); }, 100);
    };

    /* Si el usuario ya pasó el welcome pero no vio los tips (ej: recarga) */
    if (localStorage.getItem('mapita_welcomed_v1') && !localStorage.getItem(TIPS_KEY)) {
        setTimeout(function () { showTip(0); }, 1000);
    }
