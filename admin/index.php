...
<div class="container">
    ...
    <div class="tabs">
        ...
        <a href="/admin/permisos/dashboard.php" class="tab-btn <?php echo $tab==='permisos' ? 'active' : ''; ?>" style="text-decoration:none;display:inline-flex;align-items:center;">🔑 Permisos</a>
    </div>
    ...
    <div class="tab-content <?php echo $tab==='marcas' ? 'active' : ''; ?>" id="tab-marcas">
        ...
        <!-- Herramienta de geocodificación para marcas sin coords -->
        <div id="geo-lote-banner" style="display:none;background:#fffbeb;border:1px solid #fcd34d;border-radius:8px;padding:12px;margin-bottom:12px;">
            <strong style="font-size:13px;color:#92400e;">📍 Hay marcas sin ubicación en el mapa</strong>
            <p style="font-size:12px;color:#374151;margin:4px 0 8px;">
                Algunas marcas importadas no tienen coordenadas y no se ven en el mapa.
                Podés geocodificarlas automáticamente desde su campo <em>ubicación</em>.
            </p>
            <button id="btn-geo-lote" type="button"
                    onclick="geocodificarLoteMarcas()"
                    style="padding:8px 16px;background:#f59e0b;color:#fff;border:none;border-radius:6px;font-size:13px;font-weight:700;cursor:pointer;">
                📍 Geocodificar sin coords
            </button>
        </div>
...
async function loadMarcas() {
    ...
    try {
        // Usar admin_brands.php que incluye marcas SIN coordenadas (importadas sin lat/lng)
        const res    = await fetch('/api/admin_brands.php');
        const result = await res.json();
...
function renderMarcas(items) {
    ...
    // Mostrar banner de geocodificación si hay marcas sin coords en allMarcas (no en filtered)
    const banner = document.getElementById('geo-lote-banner');
    if (banner) {
        const sinCoords = allMarcas.filter(m => !m.tiene_coords).length;
        banner.style.display = sinCoords > 0 ? '' : 'none';
        const btnLote = document.getElementById('btn-geo-lote');
        if (btnLote && sinCoords > 0) {
            btnLote.textContent = '📍 Geocodificar ' + sinCoords + ' marca' + (sinCoords > 1 ? 's' : '') + ' sin coords';
        }
    }
                ...
                <tr style="border-bottom:1px solid #f0f0f0;background:${i%2===0?'white':'#fafafa'};">
                    ...
                    <td style="padding:9px 12px;text-align:center;">
                        <div style="display:flex;gap:6px;justify-content:center;flex-wrap:wrap;">
                            ...
                            ${!m.tiene_coords
                                ? `<button type="button" id="geo-btn-${m.id}"
                                       onclick="geocodificarMarcaAdmin(${m.id}, ${JSON.stringify(m.ubicacion || m.nombre)})"
                                       style="padding:5px 10px;background:#f59e0b;color:white;border:none;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;"
                                       title="Asignar coordenadas automáticamente por dirección">
                                       📍 Geocodificar
                                   </button>`
                                : ''
                            }
                        </div>
                        <div id="geo-msg-${m.id}" style="font-size:11px;margin-top:3px;"></div>
                    </td>
                </tr>`;
            }).join('')}
        </tbody>
...
/**
 * Geocodifica una marca individual desde el admin usando Nominatim.
 * Actualiza la BD via /api/update_brand_coords.php y refresca la fila en la tabla.
 */
async function geocodificarMarcaAdmin(brandId, address) {
    const btn = document.getElementById('geo-btn-' + brandId);
    const msg = document.getElementById('geo-msg-' + brandId);
    if (!address || address.trim() === '' || address.toLowerCase() === 'argentina') {
        msg.innerHTML = '<span style="color:#e53e3e;">Sin dirección para geocodificar.</span>';
        return;
    }
    if (btn) { btn.disabled = true; btn.textContent = '⏳'; }
    msg.innerHTML = '<span style="color:#6b7280;">Consultando…</span>';
    ...
    try {
        ...
        if (!data || data.length === 0) {
            msg.innerHTML = '<span style="color:#e53e3e;">❌ No encontrado. Editá la dirección.</span>';
            if (btn) { btn.disabled = false; btn.textContent = '📍 Geocodificar'; }
            return;
        }
        ...
        // Guardar en la BD
        const saveResp = await fetch('/api/update_brand_coords.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: brandId, lat: lat, lng: lng })
        });
        const saveResult = await saveResp.json();

        if (saveResult.success) {
            msg.innerHTML = '<span style="color:#059669;">✓ Guardado (' + lat.toFixed(4) + ', ' + lng.toFixed(4) + ')</span>';
            if (btn) btn.remove();
            // Actualizar la celda de coords en la misma fila
            const coordCell = document.getElementById('coord-cell-' + brandId);
            if (coordCell) {
                coordCell.innerHTML = '<span style="background:#d1fae5;color:#065f46;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:600;">✓ ' + lat.toFixed(3) + ', ' + lng.toFixed(3) + '</span>';
            }
            // Recargar lista en 1.5s para reflejar cambios
            setTimeout(loadMarcas, 1500);
        } else {
            msg.innerHTML = '<span style="color:#e53e3e;">❌ Error al guardar.</span>';
            if (btn) { btn.disabled = false; btn.textContent = '📍 Geocodificar'; }
        }
    } catch (err) {
        msg.innerHTML = '<span style="color:#e53e3e;">❌ Error de red.</span>';
        if (btn) { btn.disabled = false; btn.textContent = '📍 Geocodificar'; }
    }
}

/**
 * Geocodifica en lote todas las marcas sin coordenadas (secuencial, 1s entre requests — ToS Nominatim).
 */
async function geocodificarLoteMarcas() {
    const sinCoords = allMarcas.filter(m => !m.tiene_coords);
    if (sinCoords.length === 0) {
        alert('✅ Todas las marcas ya tienen coordenadas.');
        return;
    }
    if (!confirm('Se geocodificarán ' + sinCoords.length + ' marca(s) sin coordenadas.\nEl proceso puede tardar ~' + sinCoords.length + ' segundo(s). ¿Continuar?')) return;

    const btnLote = document.getElementById('btn-geo-lote');
    if (btnLote) { btnLote.disabled = true; btnLote.textContent = '⏳ Geocodificando…'; }

    let ok = 0, fail = 0;
    for (const m of sinCoords) {
        const address = (m.ubicacion && m.ubicacion.toLowerCase() !== 'argentina') ? m.ubicacion : m.nombre;
        await geocodificarMarcaAdmin(m.id, address);
        await new Promise(r => setTimeout(r, 1100)); // 1.1s entre requests (ToS Nominatim)
    }

    if (btnLote) { btnLote.disabled = false; btnLote.textContent = '📍 Geocodificar sin coords'; }
    alert('Proceso completado. Revisá la tabla para ver los resultados.');
    loadMarcas();
}
...
async function bulkImport(type, fileInputId, resultId) {
    ...
    try {
        ...
        // Reporte de geocodificación para marcas
        if (json.geo_report) {
            const g = json.geo_report;
            const rows = [];
            if (g.json)      rows.push('<span style="color:#059669;">📍 ' + g.json + ' con coords del JSON</span>');
            if (g.address)   rows.push('<span style="color:#0ea5e9;">📍 ' + g.address + ' geocodificada(s) por dirección</span>');
            if (g.ubicacion) rows.push('<span style="color:#6366f1;">📍 ' + g.ubicacion + ' geocodificada(s) por ubicación</span>');
            if (g.aprox)     rows.push('<span style="color:#f59e0b;">📍 ' + g.aprox + ' geocodificada(s) aproximadamente</span>');
            if (g.pending)   rows.push('<span style="color:#dc2626;">⚠️ ' + g.pending + ' sin coords — usá el botón Geocodificar en la tabla</span>');
            if (rows.length) {
                html += '<div style="margin-top:6px;padding:8px 10px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;font-size:11px;line-height:1.8;">'
                      + '<strong>Geolocalización:</strong><br>' + rows.join('<br>') + '</div>';
            }
        }

        if (json.skipped && json.skipped.length) {
            html += '<details style="margin-top:6px;"><summary style="cursor:pointer;font-size:12px;color:#92400e;font-weight:600;">⏭️ ' + json.skipped.length + ' omitidas por duplicado (ya existen)</summary>'
                  + '<ul style="margin:4px 0 0;padding-left:16px;color:#92400e;font-size:11px;">'
                  + json.skipped.map(e => '<li>' + e + '</li>').join('')
                  + '</ul></details>';
        }
