...
Cuando un JSON llega sin `lat`/`lng`, el sistema aplica una **cascada de 3 niveles**
usando [Nominatim (OpenStreetMap)](https://nominatim.openstreetmap.org/) con bias
a Latinoamérica. Si ningún nivel produce resultado, la marca se guarda **sin coordenadas**
(lat/lng = NULL) y queda visible en el Admin con alerta para geocodificación manual.

> ⚠️ **Ya no existe fallback a Buenos Aires.** Una posición falsa es peor que ninguna.
> Las marcas sin coords son fácilmente gestionables desde Admin → pestaña Marcas.

### Cascada de geocodificación para marcas

| Nivel | Campo usado | Ejemplo | Precisión |
|-------|-------------|---------|-----------|
| **1 — Dirección exacta** | `address` | `"Av. Colón 1234, Córdoba, Argentina"` | ⭐⭐⭐ Máxima |
| **2 — Ubicación específica** | `ubicacion` | `"San Miguel de Tucumán, Argentina"` | ⭐⭐ Alta |
| **3 — Nombre + rubro** | `nombre` + `rubro` + `ubicacion` | `"KAMALI, Servicios, Córdoba"` | ⭐ Aproximada |
| **NULL — Sin resultado** | — | La marca no tiene dirección resoluble | Admin la geocodifica |

**Para la IA generadora:**
- Incluir `lat`/`lng` precalculados es la opción **más eficiente** (nivel 0, sin llamada a Nominatim).
- Si no se dispone de coords, incluir `address` con la dirección del titular (nivel 1).
- Si solo se conoce la ciudad/provincia, usar `ubicacion` con ese valor (nivel 2).
- Evitar `"ubicacion": "Argentina"` — es demasiado genérico y fuerza al nivel 3.

### Regla de ubicación para marcas INPI

Al importar desde consultas INPI, el titular tiene un domicilio registrado.
Incluirlo en `address` garantiza geocodificación precisa:
...
- "address": dirección exacta del titular o sede principal (string, ej: "Av. Colón 1234, Córdoba, Argentina").
  PRIORIDAD MÁXIMA para geocodificación. Incluirlo siempre que esté disponible.
- "ubicacion": ciudad/provincia de operación (string, ej: "Córdoba, Argentina"). Default "Argentina".
  Usar un valor específico (no solo "Argentina") para mejorar la geocodificación.
- "lat": latitud decimal (número, ej: -34.6037). Si se incluye junto con "lng", el sistema NO llama a Nominatim.
- "lng": longitud decimal (número, ej: -58.3816). Incluir siempre junto con "lat".
...
4. Las fechas INPI ("inpi_fecha_registro", "inpi_vencimiento") DEBEN tener formato YYYY-MM-DD.
   Ejemplo correcto: "2023-05-20". Incorrecto: "20/05/2023", "0000-00-00" o "May 20, 2023".
   Si la fecha es desconocida, omitir el campo (no usar "0000-00-00").
...
6. REGLA DE GEOLOCALIZACIÓN (por orden de prioridad):
   a. MEJOR: incluir "lat" y "lng" con coordenadas precisas → el sistema no llama a Nominatim.
   b. MUY BUENO: incluir "address" con la dirección del titular (ej: "Av. Córdoba 1234, Rosario, Argentina").
   c. BUENO: usar "ubicacion" con ciudad y país específicos (ej: "Mendoza, Argentina").
   d. EVITAR: "ubicacion": "Argentina" sin más detalle — el sistema intentará geocodificar
      por nombre+rubro pero el resultado será aproximado o nulo.
   e. NUNCA usar "0000-00-00" ni coordenadas (0, 0) — son valores inválidos que se rechazan.
...
5. El campo `lat`/`lng` es **opcional** para ambos tipos. Si se omite, el módulo
   ...
   Si ningún nivel de la cascada produce resultado, la marca se guarda sin coords (lat/lng = NULL)
   y aparece en Admin → Marcas con botón 📍 Geocodificar para resolverlo manualmente.
...
| Geocodificación automática | ✅ Sí (Nominatim, cascada 3 niveles) | ✅ Sí (Nominatim, cascada 3 niveles) |
