            ...
            try {
                ...
                if ($type === 'brands') {
                    ...
                    foreach ($validItems as $i => $row) {
                        $rowNum = $i + 1;
                        try {
                            // ── Chequeo de duplicados contra la BD ───────────
                            if (!empty($row['inpi_numero'])) {
                                $chk = $db->prepare("SELECT id FROM brands WHERE inpi_numero = ? LIMIT 1");
                                $chk->execute([mb_substr((string)$row['inpi_numero'], 0, 50)]);
                                if ($existId = $chk->fetchColumn()) {
                                    $importSkipped[] = "Fila {$rowNum} ('{$row['nombre']}'): ya existe con INPI N° {$row['inpi_numero']} (ID #{$existId}).";
                                    continue;
                                }
                            }
                            $chkN = $db->prepare("SELECT id FROM brands WHERE LOWER(TRIM(nombre)) = LOWER(TRIM(?)) LIMIT 1");
                            $chkN->execute([(string)$row['nombre']]);
                            if ($existId = $chkN->fetchColumn()) {
                                $importSkipped[] = "Fila {$rowNum} ('{$row['nombre']}'): ya existe con ese nombre (ID #{$existId}).";
                                continue;
                            }
                ...
                $report['import_skipped'] = $importSkipped ?? [];
                $importDone = true;
                $step = 'done';
...
<div class="container">
    ...
    <div class="card">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-box warn">
                <div class="num"><?php echo count($report['import_skipped'] ?? []); ?></div>
                <div class="lbl">Omitidos (ya existen)</div>
            </div>
            <div class="stat-box err">
        ...
        <?php if (!empty($report['import_skipped'])): ?>
        <div class="alert alert-warning">
            <strong>⏭️ Omitidos por duplicado — ya existen en la base de datos:</strong>
            <ul>
                <?php foreach ($report['import_skipped'] as $is) echo '<li>' . htmlspecialchars($is) . '</li>'; ?>
            </ul>
        </div>
        <?php endif; ?>
