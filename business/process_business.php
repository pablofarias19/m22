...
function mapitaAllowedBusinessTypes(): array {
    return [
        ...
        'supermercado','comercio','autos_venta','motos_venta','indumentaria','verduleria','carniceria','pastas','ferreteria','electronica','muebleria','floristeria','libreria',
...
function deleteBusiness($businessId, $userId) {
    try {
        ...
        // Eliminar solo lo permite el titular o superadmin — NO un admin delegado
        $isTitular = (int)$business['user_id'] === $userId;
        if (!$isTitular && !isAdmin()) {
            return ['success' => false, 'message' => 'Solo el titular puede eliminar el negocio.'];
        }
