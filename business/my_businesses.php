...
try {
    ...
    if ($isAdmin) {
        $stmt = $db->prepare("
            SELECT b.*, u.username AS owner_name, 0 AS es_delegado
            FROM businesses b
        ...
        // Negocios propios + negocios donde el usuario es admin delegado
        $stmt = $db->prepare("
            SELECT b.*, u.username AS owner_name, 0 AS es_delegado
            FROM businesses b
            LEFT JOIN users u ON b.user_id = u.id
            ...
            SELECT b.*, u.username AS owner_name, 1 AS es_delegado
            FROM businesses b
            LEFT JOIN users u ON b.user_id = u.id
            INNER JOIN business_delegations bd
        ...
        <div class="biz-card">
            ...
            <div class="card-header">
                ...
                <div class="card-title-block">
                    ...
                    <?php endif; ?>
                    <?php if (!empty($b['es_delegado'])): ?>
                    <span style="font-size:10px;font-weight:700;background:#ede9fe;color:#6d28d9;
                                 border:1px solid #c4b5fd;border-radius:10px;padding:2px 7px;margin-top:3px;display:inline-block;"
                          title="Tenés permisos de administración delegados por el titular">
                        🔑 Delegado
                    </span>
                    <?php endif; ?>
                    <?php if (($isAdmin || !empty($b['es_delegado'])) && !empty($b['owner_name'])): ?>
