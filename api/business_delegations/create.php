...
$bizName  = (string)($business['name'] ?? ('Negocio #' . $businessId));
$owner    = mapitaGetUserContactById($db, (int)($business['user_id'] ?? 0));

// ── Notificación al DELEGADO: recibe el acceso ────────────────────────────
mapitaSendUserNotificationEmail(
    $targetUser['email'] ?? null,
    'MAPITA | Te dieron acceso de administración a un negocio',
    'Acceso de administración delegado',
    [
        'Negocio'       => $bizName,
        'Titular'       => (string)($owner['username'] ?? ('Usuario #' . ($business['user_id'] ?? '?'))),
        'Tu usuario'    => (string)($targetUser['username'] ?? ('Usuario #' . $delegateUserId)),
        'Acceso desde'  => date('d/m/Y H:i'),
        'Podés administrar este negocio desde' => 'https://mapita.com.ar/mis-negocios',
    ]
);

// ── Confirmación al TITULAR: operación registrada ─────────────────────────
mapitaSendUserNotificationEmail(
    $owner['email'] ?? null,
    'MAPITA | Confirmación: delegaste administración de un negocio',
    'Confirmación de delegación',
    [
        'Negocio'   => $bizName,
        'Delegado a' => (string)($targetUser['username'] ?? ('Usuario #' . $delegateUserId)),
        'Email delegado' => (string)($targetUser['email'] ?? '—'),
